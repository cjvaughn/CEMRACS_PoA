...
\begin{proof}
	...
	\end{split}.
	\end{equation*}
	We can deduce that $u^{\infty,r}$ over $[0,T]$.	Similarly, for any fixed time $t\in [0,T]$, the real solution $w_t$ and $\eta_t$ of the equations (\ref{eq:riccati_wt}) and (\ref{eq:riccati_etat}) at time $t$ converge towards the solution $w^{\infty,r}_t$ and $\eta_t^{\infty,r}$ of some first-order differential equations. Namely, for every $t\in [0,T]$,
	...
	Hence, by the bounded convergence theorem again we obtain
	\begin{equation*}
	...
	\end{equation*}
	Thus, from Proposition \ref{prop:diff_SC} and equation (\ref{eq:PoA_LQ}), we deduce
	$$ \frac{(b_2 + \bar{b}_2)^2}{r + \bar{r}(1 - \bar{s})^2} \longrightarrow 0$$ 
	and thus $\displaystyle \lim_{r \to \infty} PoA = 1$. 
