...
\begin{proof}
	...
	\begin{IEEEeqnarray*}{rl}
		...
		&\leq  c_1 + 2 c_1 \sqrt{(A^u)^2 + c_1^2} \\
		&< \infty,
	\end{IEEEeqnarray*}
	and 
	\begin{IEEEeqnarray*}{rl}
	...
	\begin{IEEEeqnarray}{rCl}
		\left\vert u_t^{r} \right\vert  &=& \frac{\left\vert (C^{u,r} + D^{u,r} \delta^{+,r}_u) - (C^{u,r} + D^{u,r} \delta^{-,r}_u) e^{- (\delta^{+,r}_u - \delta^{-,r}_u) (T-t)} \right\vert}{\left\vert ( -\delta^{-,r}_u + D^{u,r} B^{r} ) + (\delta^{+,r}_u - D^{u,r} B^{r} ) e^{-(\delta^{+,r}_u - \delta^{-,r}_u)(T-t)} \right\vert } \nonumber \\
...
\begin{proof}
	...
	otherwise when $b_2 \to \infty$ we have
	$ \lambda^{b_2} \to \lambda^{b_2 \to \infty}$.	
	For other coefficients, as long as $b_2 \to \infty$ we have 
	\begin{equation*}
	\begin{array}{l}
	...
	\end{equation*}
	Thus $\delta^{+,b_2}_u - \delta^{-,b_2}_u \xrightarrow[b_2 \to \infty]{} +\infty$ and for any $0 \leq t < T$:
...
%	$$ b_2 \eta_t^{b_2} \xrightarrow[b_2 \to \infty]{} (r+ \bar{r}) c_{\delta_\eta^{b_2 \to \infty}} =: c_5, \qquad \text{with } \quad c_{\delta_\eta^{b_2 \to \infty}} = \sqrt{\frac{q+\bar{q}}{r + \bar{r}}}.
%	$$
	
	% lower bound for (b_2 + b2 bar) u_t
	Moreover, we can derive a uniform lower bound for $(b_2 + \bar{b}_2) u_t^{b_2}$ for any large enough $b_2$ by expanding this whole expression with equation (\ref{eq:riccati_ut_sol}). Let $\epsilon_1 > 0$ so that $\epsilon_1 < \min \{ c_{\delta_u^{b_2 \to \infty}}, D^{u,b_2 \to \infty} \}$, for example $\epsilon_1 = \frac{1}{2} \min \{ c_{\delta_u^{b_2 \to \infty}}, D^{u,b_2 \to \infty} \}$. Then there exists $b_2^{*,lower} := b_2^{*,lower}(\epsilon_1) 	>0$ such that for all $b_2 \geq b_2^{*,lower}$,
	$$ 
	...
	\begin{IEEEeqnarray}{rCl}
		...
		&=& \frac{D^{u,b_2} \frac{\delta_u^{+, b_2}}{b_2+ \bar{b}_2} }{ \frac{\delta_u^{+,b_2} - \delta_u^{-,b_2}}{b_2 + \bar{b}_2} + \frac{D^{u,b_2}}{r + \bar{r}(1-\bar{s})^2} } \nonumber \\
		&\geq& \frac{(D^{u,b_2 \to \infty} - \epsilon_1 ) \cdot (c_{\delta_u^{b_2 \to \infty}} - \epsilon_1) }{ (2 c_{\delta_u^{b_2 \to \infty}} + 2 \epsilon_1) + (D^{u,b_2 \to \infty} + \epsilon_1) / (r+ \bar{r}(1-\bar{s})^2) } =: m_u(\epsilon_1)
	...
	\end{equation*}
	Hence, the function $t \mapsto u_t^{b_2}$ is increasing when $B^{b_2} (D^{u,b_2})^2 + 2 A^u D^{u,b_2} - C^{u,b_2} > 0$. Since 
	$$ B^{b_2} (D^{u,b_2})^2 + 2 A^u D^{u,b_2} - C^{u,b_2} \xrightarrow[b_2 \to \infty]{} \infty,$$
	there must exists $b_2^{*,u,upper}>0$ such that for all $b_2 \geq b_2^{*,u,upper}$, we have $t \mapsto u_t^{b_2}$ being increasing and 
	...
	Idem for $t \mapsto w_t^{b_2}$. Let $b_2^{*,upper} = \max \{ b_2^{*,u,upper}, b_2^{*,w,upper}\}$ and $M = \max \{  D^{u,b_2 \to \infty}+1, D^w+1\}$, we get a uniform upper bound for $u_t^{b_2}$ and $w_t^{b_2}$ for all $b_2\geq b_2^{*,upper}$, namely
	\begin{equation}
	\sup_{b_2 \geq b_2^{*,upper}} \sup_{0 \leq t \leq T} \max \left\{ u_t^{b_2} , w_t^{b_2}\right\} \leq M.
	\label{eq:ut_wt_upper_bound_b2_infty}
	\end{equation}
	
	Now we are ready to show that $\lim_{b_2 \to \infty} \frac{\Delta SC}{SC^{MKV}} = 0$. The idea is to split the whole interval into two parts $[0,T] =[0,t^*] \cup [t^*,T]$.	
	Since $v_t \geq 0$ for all $t\in [0,T]$, we have $SC^{MKV} \geq w_0^{b_2} \mathbb{E}(\xi)^2$.	For the sake of simplicity, for all $t \in [0,T]$, we denote $h_u(b_2,t) = (b_2 + \bar{b}_2) u_{t}^{b_2}$ and $h_w(b_2,t) = (b_2 + \bar{b}_2)w_t^{b_2}$. Thus by multiplying $(b_2 + \bar{b}_2)$ both on the numerator and denominator we get
	\begin{IEEEeqnarray*}{rCl}
		0 \leq \frac{\Delta SC}{SC^{MKV}} &\leq& \frac{1}{w_0^{b_2} \mathbb{E}(\xi)^2 } \left( B^{b_2} \int_{[0,t^*]} (u_t^{b_2} - w_t^{b_2})^2 (\bar{x}_t^{MFG,b_2})^2 dt + B^{b_2} \int_{[t^*, T]} (u_t^{b_2} - w_t^{b_2})^2 (\bar{x}_t^{MFG,b_2})^2 dt \right) \\ 
		&=&  \frac{1}{(b_2 + \bar{b}_2) w_0^{b_2} } \left( I_1 + I_2 \right)
	\end{IEEEeqnarray*}
	where
	\begin{IEEEeqnarray*}{rCl}
	I_1 &=& \frac{(b_2 + \bar{b}_2)^3}{r + \bar{r}(1-\bar{s})^2}  \int_0^{t^*} (u_t^{b_2} - w_t^{b_2})^2 e^{2(b_1 + \bar{b}_1)t} \exp \left( -2B^{b_2} \int_0^{t} u_s^{b_2} ds 
	\right) dt \\
	&=& \frac{b_2 + \bar{b}_2}{r + \bar{r}(1-\bar{s})^2} \int_0^{t^*} [ h_u(b_2,t)- h_w(b_2,t) ]^2 \cdot e^{2(b_1 + \bar{b}_1)t} \exp \left( - \frac{2 (b_2 + \bar{b}_2)}{r+ \bar{r}(1-\bar{s})^2}  \int_0^t h_u(b_2,s) ds \right) dt	 
	\end{IEEEeqnarray*}
	and 
	$$
		I_2 = \frac{(b_2 + \bar{b}_2)^3}{r + \bar{r}(1-\bar{s})^2}  \int_{t^*}^T (u_t^{b_2} - w_t^{b_2})^2 e^{2(b_1 + \bar{b}_1)t} \exp \left( -\frac{2 (b_2 + \bar{b}_2)}{r + \bar{r}(1-\bar{s})^2} \int_0^{t} h_u(b_2,s) ds \right) dt.
	$$	
	The time point $t^*$ is chosen afterwards in order to make $I_1$ small enough. For any given $\epsilon>0$, we want to show that $I_1 \leq \epsilon$ and $I_2 \leq \epsilon$ for any $b_2$ large enough.\\
	  

	% the term I_1
	We look at the term $I_1$ first. Recall that on the closed sub-interval $[0,T/2]$, we have pointwise convergence $(b_2 + \bar{b}_2) u_t^{b_2} \to c_u$ and $(b_2 + \bar{b}_2) w_t^{b_2} \to c_w$. Because we have shown that for all $b_2 \geq b_w^{*,upper}$, the functions $[0,T/2] \ni s \mapsto u_s^{b_2}$ and $[0,T/2] \ni s \mapsto w_s^{b_2}$ are increasing. Consequently, for any positive constant $\epsilon_2 < \min \{ c_u, c_w \} $, saying for example $\epsilon_2 = \frac{1}{2} \min \{c_u, c_w \}$, there exists $b_2^{*,I_1} := b_2^{*,I_1}(\epsilon_2) > b_2^{*,upper}$ such that for all $b_2 \geq b_2^{*,I_1}$ and all $s\in [0, T/2]$ we have:
	\begin{equation*}
	\left\{
	\begin{array}{lclclclcl}
		c_u - \epsilon_2 &\leq& h_u(b_2,0) &\leq& h_u(b_2,s) &\leq& h_u(b_2, T/2 ) &\leq& c_u + \epsilon_2 \\
		c_w - \epsilon_2 &\leq& h_w(b_2,0) &\leq& h_w(b_2,s) &\leq& h_w(b_2, T/2 ) &\leq& c_w + \epsilon_2 
	\end{array}
	\right.
	\end{equation*}
	Thus, if $t^* < \frac{T}{2}$, then for any $t \in [0,t^*] \subset [0,T/2]$ and $b_2 \geq b_2^{*,I_1}$,
	$$
		\vert h_u(b_2, t) - h_w(b_2,t) \vert^2 \leq  2 [ (c_u + \epsilon_2)^2 + (c_w+ \epsilon_2)^2 ]\quad \text{and} \quad \int_0^t h_u(b_2,s)ds \geq (c_u - \epsilon_2) t,
	$$
	which implies
	\begin{IEEEeqnarray}{rCl}
		I_1 &=& \frac{b_2 + \bar{b}_2}{r + \bar{r}(1-\bar{s})^2} \int_0^{t^*} [ h_u(b_2,t)- h_w(b_2,t) ]^2 \cdot e^{2(b_1 + \bar{b}_1)t} \exp \left( - \frac{2 (b_2 + \bar{b}_2)}{r+ \bar{r}(1-\bar{s})^2}  \int_0^t h_u(b_2,s) ds \right) dt \nonumber \\
		&\leq& \frac{e^{(b_1 + \bar{b}_1) T}\cdot  2[ (c_u+ \epsilon_2)^2 + (c_w + \epsilon_2)^2 ]}{r+ \bar{r}(1-\bar{s})^2} \cdot (b_2 + \bar{b}_2) \int_0^{t^*} \exp \left(  - \frac{2 (b_2 + \bar{b}_2)}{r + \bar{r}(1-\bar{s})^2} \cdot (c_u - \epsilon_2) t \right) dt \nonumber \\
		&=& \kappa_1 \left( 1 - e^{ - \kappa_2 \cdot (b_2 + \bar{b}_2) t^* } \right) 
	\label{eq:I1_ineq_b2_infinity}
	\end{IEEEeqnarray}
	where the coefficients $ \displaystyle \kappa_1 := \kappa_1(\epsilon_2) = \frac{e^{(b_1 + \bar{b}_1) T}\cdot  [ (c_u+ \epsilon_2)^2 + (c_w + \epsilon_2)^2 ]}{c_u - \epsilon_2}$ and $\displaystyle \kappa_2 := \kappa_2(\epsilon_2) = \frac{2 (c_u - \epsilon_2)}{r + \bar{r}(1-\bar{s})^2} $ independent of $b_2$.	
	In particular, the above inequality is valid when $b_2 = b_2^{*,I_1}$, so if we let $\epsilon_2 = \frac{1}{2} \min \{c_u, c_w\}$ as previous, and choose the time point $t^*$ as following
	\begin{equation}
	 t^* := t^*(\epsilon) = \min\left\{ \frac{T}{2}, \  \frac{1}{\kappa_2 \cdot \left( b_2^{*,I_2} + \bar{b}_2\right) } \ln \left( \frac{1}{\displaystyle 1- \frac{\epsilon}{\kappa_1} } \right) \right\},
	\label{eq:t^*_b2_infty} 
	\end{equation}
	then we are able to deduce from inequality (\ref{eq:I1_ineq_b2_infinity}) that
	\begin{equation}
		I_1 \leq \kappa_1 \left[1 - e^{- \kappa_2 \cdot (b_2^{*,I_2} + \bar{b}_2) \cdot t^*}  \right] = \epsilon.\\
	\label{eq:I1_b2_infty}
	\end{equation}
	
	% check I_2
	Now we look at the term $I_2$ with the above given $t^*(\epsilon)$. 
	Since the functions $s \mapsto u_s^{\bar{b}_2}$, $s \mapsto w_s^{\bar{b}_2}$ and $s \mapsto \bar{x}_s^{MFG,\bar{b}_2}$ are continuous over $[t^*,T]$, then the functions $\displaystyle t \mapsto \int_0^t u^{\bar{b}_2}_s ds$ and $\displaystyle t \mapsto \int_0^t (u^{\bar{b}_2}_s- w^{\bar{b}_2}_s)^2 (\bar{x}_s^{MFG,\bar{b}_2} )^2 ds$ are continuous over $[t^*,T]$ and differentiable over $(t^*,T)$. We can apply the Mean Value theorem and deduce that there exists a time points $t_1$ depending on $b_2$ which satisfy $t^* < t_1 < T$ such that:
	\begin{IEEEeqnarray}{rCl}
		I_2 &=& \frac{(b_2 + \bar{b}_2)^3}{r + \bar{r}(1-\bar{s})^2}  \int_{t^*}^T (u_t^{b_2} - w_t^{b_2})^2 e^{2(b_1 + \bar{b}_1)t} \exp \left( -\frac{2 (b_2 + \bar{b}_2)}{r + \bar{r}(1-\bar{s})^2} \int_0^{t} h_u(b_2,s) ds \right) dt \nonumber \\
		&=&  \frac{(u_{t_1}^{b_2} - w_{t_1}^{b_2})^2 e^{2(b_1 + \bar{b}_1)t_1} }{r + \bar{r}(1-\bar{s})^2}  \cdot  (b_2 + \bar{b}_2)^3 e^{ -\frac{2 (b_2 + \bar{b}_2)}{r + \bar{r}(1-\bar{s})^2} \int_0^{t_1} h_u(b_2,s) ds }.
	\label{eq:I2_b2_infity_Mean_Value}
	\end{IEEEeqnarray}	
	We know from equation (\ref{eq:ut_uniform_lower_bound_b2_infinity}) and (\ref{eq:ut_wt_upper_bound_b2_infty}) that for all $b_2 \geq \max \{b_2^{*,upper}, b_2^{*,lower} \}$ and all $t \in [0,T]$:
	\begin{equation*}
		\vert u_t^{b_2} - w_t^{b_2} \vert \leq \sup_{0 \leq t \leq T} \vert u_t^{b_2} \vert + \vert w_t^{b_2} \vert  \leq 2M
	\end{equation*}
	and 
	\begin{equation*}
		\int_0^{t_1} h_u(b_2,s) ds \geq \int_0^{t^*} h_u(b_2,s) ds \geq m_u(\epsilon_1) \cdot t^*.
	\end{equation*}
	Together with equation (\ref{eq:I2_b2_infity_Mean_Value}) give raise to
	\begin{equation*}
		I_2 \leq  \frac{4 M^2 e^{2(b_1 + \bar{b}_1) T} }{r + \bar{r}(1-\bar{s})^2} (b_2 + \bar{b}_2)^3 e^{- \frac{2 t^*  m_u(\epsilon_1)}{r + \bar{r}(1-\bar{s})^2} (b_2 + \bar{b}_2) } = \kappa_3 (b_2 + \bar{b}_2)^3 e^{- \kappa_4 (b_2 + \bar{b}_2)} 
	\end{equation*}
	where $\kappa_3 = \frac{4 M^2 e^{2(b_1 + \bar{b}_1) T} }{r + \bar{r}(1-\bar{s})^2} $ and $\kappa_4 := \kappa_4(\epsilon, \epsilon_1) =  \frac{2 t^*  m_u(\epsilon_1)}{r + \bar{r}(1-\bar{s})^2}$ are two positive constants independent of $b_2$. 	
	Hence, there exists $b_2^{*,I_2} := b_2^{*,I_2}(\epsilon, \epsilon_1) > \max \{b_2^{*,upper}, b_2^{*,lower} \}$ such that for all $b_2 \geq b_2^{*,I_2}$, 
	\begin{equation}
		I_2 \leq \kappa_3 (b_2 + \bar{b}_2)^3 e^{- \kappa_4 (b_2 + \bar{b}_2)} \leq \epsilon.
	\label{eq:I2_b2_infty}
	\end{equation}
	
	Let $\epsilon_1, \epsilon_2$ be two constants as follow:
	$$ \epsilon_1 = \frac{1}{2} \min \{ c_{\delta_u^{b_2 \to \infty}}, D^{u,b_2 \to \infty} \}\quad \text{and} \quad \epsilon_2 = \frac{1}{2} \min \{c_u, c_w\}.$$ Hence, 
	for any $\epsilon>0$, we can choose $t^*$ as shown in equation (\ref{eq:t^*_b2_infty}) so that for all $b_2 \geq b_2^*(\epsilon) := \max \{ b_2^{*,I_1}, b_2^{*,I_2}(\epsilon) \}$, the inequalities (\ref{eq:I1_b2_infty}) and (\ref{eq:I2_b2_infty}) yield
	\begin{equation*}
		\frac{\Delta SC}{SC^{MKV}} \leq \frac{I_1 + I_2}{(b_2 + \bar{b}_2) w_0^{b_2}} \leq \frac{ \epsilon + \epsilon }{h_w(b_2,0)} \leq \frac{2\epsilon}{c_w - \epsilon_2} 
	\end{equation*}
	Therefore, by letting $\epsilon \to 0$, we conclude that:
	\begin{equation*}
		\lim_{b_2 \to \infty} \frac{\Delta SC}{SC^{MKV}} = 0, 
	\end{equation*}
...
\begin{comment}
	...
	\begin{IEEEeqnarray}{rCl}
		\Delta SC &=& \frac{1}{2} B^{b_2} \int_0^T (u^{b_2}_t- w^{b_2}_t)^2 (\bar{x}_t^{MFG,b_2} )^2 dt  \nonumber \\
		&=& \frac{1}{2} B^{b_2}  (u_{t_1}^{b_2}- w_{t_1}^{b_2} )^2 (\bar{x}_{t_1}^{MFG,b_2} )^2  \cdot T \nonumber \\
		&=& \frac{T  B^{b_2} }{2} \cdot (u_{t_1}^{b_2}- w_{t_1}^{b_2} )^2 \cdot \mathbb{E}[\xi]^2 \exp \left( 2 (b_1 + \bar{b}_1) t_1 - 2\int_0^{t_1} B^{b_2} u_s^{b_2} ds \right) \nonumber \\
	...
	\begin{IEEEeqnarray}{rCl}
		...
		&=&  Var(\xi) e^{2b_1 T} e^{-\frac{2T}{r+\bar{r}} b_2 h_\eta(b_2,t_4)} + \sigma^2 T e^{2b_1 (T-t_5)} \exp \left( -2 B^{\eta,b_2} \eta_{t_6}^{b_2} (T-t_5) \right) \nonumber \\
	...
	\begin{comment}
	Idem, there exists 3 time points $t_7,t_8,t_9$ with $0<t_7<t_3<T$ and $0<t_8<t_9<t_3<T$ such that
	\begin{equation}
		v^{b_2}_{t_3} = \kappa_8 e^{-\kappa_7 b_2 h_\eta(b_2,t_7)} + \kappa_{10} e^{-\kappa_9 b_2 h_\eta(b_2,t_9)}
	\label{eq:v_{t_3}_b_2_expansion}
	\end{equation}
	where $ \displaystyle \kappa_7 = \frac{2t_3}{r+ \bar{r}}, \  \kappa_8 =Var(\xi) e^{2b_1 t_3}, \  \kappa_9 = \frac{2 (t_3 - t_8)}{r+ \bar{r}}, \  \kappa_{10}= \sigma^2 t_3 e^{2b_1 (t_3 - t_8)}$.
	%%%%%%% end{comment}
	...
	\begin{IEEEeqnarray}{rCl}
		\frac{\Delta SC}{SC^{MKV}} &=& \frac{\displaystyle \kappa_1 \left(u_{t_1}^{b_2}- w_{t_1}^{b_2} \right)^2 \cdot (b_2 + \bar{b}_2)^2 e^{-\kappa_2 (b_2+ \bar{b}_2)h_u(b_2,t_2)} }{\displaystyle (q + \bar{q} ) T v_{t_3}^{b_2} +  \frac{T}{r + \bar{r}} (\eta_{t_3}^{b_2})^2 \cdot b_2^2 v_{t_3}^{b_2} + (q_T + \bar{q}_T) v_T^{b_2} + \frac{1}{b_2+\bar{b}_2} (\mathbb{E}(\xi) )^2 h_w(b_2,0)  } \nonumber \\
		&=& \frac{l_{u,v}(b_2) \cdot (b_2 + \bar{b}_2)^3 e^{- \kappa_2 (b_2 + \bar{b}_2) h_u(b_2,t_2)}} { l_{\eta,1}(b_2) \cdot b_2 v_{t_3}^{b_2}  + l_{\eta,2}(b_2)\cdot b_2^3 v_{t_3}^{b_2}+ l_{\eta,3}(b_2) \cdot b_2 v_T^{b_2} + (\mathbb{E}(\xi))^2 h_w(b_2,0) }
	...
	\begin{comment}
	With the limits of $h_u(b_2,t), h_w(b_2,t)$ and $h_\eta(b_2,t)$ for any fixed $t \in [0,T)$, it is easy to check that 
	$$ l_{u,v}(b_2,t_3) \xrightarrow[b_2 \to \infty]{} \kappa_1 (c_3 - c_4)^2, \quad l_\eta(b_2) \xrightarrow[b_2 \to \infty]{} (q+ \bar{q} + \frac{c_5^2}{r+ \bar{r}})T,\quad l_1(b_2) \xrightarrow[b_2 \to \infty]{} q_T + \bar{q}_T.$$
	%%%%%%% end{comment}
...
\begin{proof}
	If $\bar{b}_2 =0$, we have 
	\begin{equation*}
		B^{b_2} \xrightarrow[b_2 \to 0]{} 0, \quad B^{\eta,b_2} \xrightarrow[b_2 \to 0]{} 0, \quad \lambda = \frac{r + \bar{r}(1- \bar{s})^2}{r + \bar{r}(1-\bar{s})}
	\end{equation*}
	and $(A^{u},C^{u}, D^{u}), (A^{w}, C^{w}, D^{w}), (A^\eta, C^\eta, D^\eta)$ are all independent of $b_2$. We can then use the same technique shown in Proposition \ref{prop:r_bar_r} to conclude that 
	$$\lim_{b_2 \to 0} PoA = 1.$$ 
	...
	\end{equation*}
	and $A^u, (A^w, C^w, D^w), (A^\eta, C^\eta, D^\eta)$ are independent of $b_2$.
	Moreover, we have $\delta^{+,b_2}_u \xrightarrow{} -2A^u >0 $ and $\delta^{-,b_2}_u \to 0$, so that $\delta^{+}_u - \delta^{-}_u \xrightarrow{} -2 A^u > 0$. Thus, from equation (\ref{eq:riccati_ut_sol}) we deduce that for every fixed time $t \in [0,T]$:
	$$u^{b_2}_t \xrightarrow[b_2 \to 0 ]{} 0 .$$
	Similar to the case in Proposition \ref{prop:r_bar_r}, we can derive a uniform bound for $u_t^{b_2}$ over $[0,T]$ for any $b_2$ small enough. Indeed, for any fixed $\epsilon >0$ there exits $b_2^* >0$ such that for any $b_2 \leq b_2^*$, $\max \{B^{b_2}, C^{u,b_2}, D^{u,b_2} \} \leq B^{b_2 \to \infty} + \epsilon =: c_1$ and
	\begin{equation*}
	 \vert u_t^{b_2} \vert \leq \frac{c_1 + 2 \epsilon \sqrt{(A^u)^2 + c_1^2} }{- 2 A^u e^{-2T \sqrt{(A^u)^2 + c_1^2} } } =: c_2 ,\qquad \forall  \  t \in [0,T]
	\end{equation*}
	
	From equation (\ref{eq:x_bar_MFG_explicit_new_notation}), we have $\displaystyle \bar{x}_t^{MFG,b_2}=\mathbb{E}(\xi) \exp \left( \int_0^t(b_1+\bar{b}_1-B^{b_2} u^{b_2}_s)ds\right)$, and by the bounded convergence theorem we derive that for any fixed $t \in [0,T]$:
	$$\bar{x}_t^{MFG,b_2} \xrightarrow[b_2 \to \infty]{} \mathbb{E}(\xi) e^{(b_1+\bar{b}_1)t} =: \bar{x}_t^{MFG,b_2 \to 0} > 0$$
	
	
	Moreover, since $B^{b_2 \to 0}>0, B^{b_2 \to 0}C^w >0, B^{b_2\to 0}D^w \geq 0$, the limiting solution for $w_t^{b_2}$ is still well defined, denoted by $w_t^{b_2 \to 0}$, which is strictly positive over $[0,T]$. We also have $w_t^{b_2}$ is uniformly bounded over $[0,T]$ for all $b_2$ small enough.
	...
	we deduce that $\eta_t^{b_2}$ is uniformly bounded over $[0,T]$ for any $b_2$ small enough and $\eta_t^{b_2}$ converge pointwisely to a strictly positive function $\eta_t^{b_2 \to 0}$. More precisely, we can have
	$$\eta_t^{b_2 \to 0} := \left(D^{\eta} - \frac{C^{\eta}}{2A^\eta}\right) e^{-2A^\eta(T-t)} + \frac{C^\eta}{2A^{\eta} }.$$
	Again, with equation (\ref{eq:v_t_new_notation}) and the Bounded Convergence theorem, the variance function $v_t^{b_2}$ converges pointwisely to some strictly positive continuous function $\displaystyle v_t^{b_2 \to 0} := Var(\xi) e^{2b_1 t} + \sigma^2 \int_0^t e^{2b_1 (t-s)} ds$. We can then obtain that
...
%	$$\lim_{\bar{b}_2 \to \infty} \delta^{\pm,\bar{b}_2}_u=\pm \infty \qquad \text{and } \qquad \lim_{\bar{b}_2 \to \infty} \frac{\delta^{\pm,\bar{b}_2}_u}{b_2 + \bar{b}_2}=0.$$
	...
	\begin{IEEEeqnarray*}{rCl}
		\vert u_t^{\bar{b}_2} \vert &=& \frac{\displaystyle C^{u,\bar{b}_2} \left(1 - e^{-(\delta^{+,\bar{b}_2}_u - \delta^{-,\bar{b}_2}_u)(T-t)} \right) + D^{u,\bar{b}_2} \left(\delta_u^{+,\bar{b}_2} - \delta_u^{-,\bar{b}_2} e^{- (\delta^{+,\bar{b}_2}_u - \delta^{-,\bar{b}_2}_u)(T-t)} \right) }{\displaystyle B^{\bar{b}_2} D^{u,\bar{b}_2} \left( 1 - e^{-(\delta^{+,\bar{b}_2}_u - \delta^{-,\bar{b}_2}_u)(T-t)} \right) + \left( - \delta_u^{-,\bar{b}_2} + \delta_u^{+,\bar{b}_2} e^{- (\delta^{+,\bar{b}_2}_u - \delta^{-,\bar{b}_2}_u)(T-t)} \right) } \\
		&\leq & \frac{C^{u,\bar{b}_2}+ D^{u,\bar{b}_2} \left(\delta_u^{+,\bar{b}_2} - \delta_u^{-,\bar{b}_2} \right)}{- \delta_u^{-,\bar{b}_2}} \\
		&\leq & \frac{\epsilon_1 + \epsilon_1 (2 c_{\delta_u^{\bar{b}_2 \to \infty}} + 2 \epsilon_1)}{ c_{\delta_u^{\bar{b}_2 \to \infty}} - \epsilon_1} =: M_u(\epsilon_1)
	\end{IEEEeqnarray*}
	...
	\begin{IEEEeqnarray}{rCl}
		\left\vert (b_2 + \bar{b}_2)^{\frac{3}{2}} u_t^{\bar{b}_2} \right\vert &\geq&   (b_2 + \bar{b}_2)^{\frac{3}{2}} \frac{D^{u,\bar{b}_2} \delta_u^{+,\bar{b}_2}}{(\delta_u^{+,\bar{b}_2} - \delta_u^{-,\bar{b}_2}) + B^{\bar{b}_2} D^{u,\bar{b}_2} }  \nonumber \\
		&= & \frac{\zeta \frac{\delta_u^{+,\bar{b}_2}}{\sqrt{b_2 + \bar{b}_2}} }{\frac{1}{\sqrt{b_2 + \bar{b}_2}} \cdot \frac{\delta_u^{+,\bar{b}_2}- \delta_u^{-,\bar{b}_2} }{\sqrt{b_2 + \bar{b}_2} } + \frac{\zeta}{r + \bar{r}(1- \bar{s})^2}  } \nonumber \\
		&\geq & \frac{\zeta (c_{\delta_u^{\bar{b}_2 \to \infty}} - \epsilon_1)}{ \epsilon_1 (2 c_{\delta_u^{\bar{b}_2 \to \infty}} + 2 \epsilon_1) + \zeta / (r+ \bar{r}(1-\bar{s})^2) } =: m_u(\epsilon_1)
	\label{eq:u_t_lower_bound_b2_bar_infty}
	\end{IEEEeqnarray} 
	
	Now, we are ready to prove $\lim_{\bar{b}_2 \to \infty} \Delta SC = 0$. For any given $\epsilon > 0$, 
	%we split the interval $[0,T]$ into two parts $[0,t_\epsilon]$ and $ [t_\epsilon, T]$ in which $0< t_\epsilon < T$ is a time point depending on $\epsilon$, 
	...
	\begin{IEEEeqnarray}{rCl}
		I_1 &=& \frac{1}{2} B^{\bar{b}_2} \int_0^{\frac{T}{2}}  (u^{\bar{b}_2}_t- w^{\bar{b}_2}_t)^2 (\bar{x}_t^{MFG, \bar{b}_2} )^2 dt  \nonumber \\
		&=& \frac{\mathbb{E}(\xi)^2}{2 (r + \bar{r}(1-\bar{s})^2) } \int_0^{\frac{T}{2}} \left( (b_2 + \bar{b}_2) u_t^{\bar{b}_2} - (b_2 + \bar{b}_2) w_t^{\bar{b}_2} \right)^2 e^{2(b_1 + \bar{b}_1) t} \cdot e^{ -\frac{2}{r + \bar{r}(1-\bar{s})^2} (b_2 + \bar{b}_2)^{\frac{1}{2}} \int_0^t (b_2 + \bar{b}_2)^{\frac{3}{2}} u_s^{\bar{b}_2} ds} dt \nonumber \\
		&\leq& \frac{\mathbb{E}(\xi)^2 [2 \epsilon_2^2 + 2 (c_w + \epsilon_2)^2 ] e^{2(b_1 + \bar{b}_1) \frac{T}{2}} }{2( r + \bar{r}(1-\bar{s})^2)} \int_0^{\frac{T}{2}} \exp \left(- \frac{2}{r + \bar{r}(1-\bar{s})^2} (b_2 + \bar{b}_2)^{\frac{1}{2}} \cdot (c_u - \epsilon_2) t \right) dt \nonumber \\
		&=& \kappa_1(\epsilon_2) \frac{1}{(b_2 + \bar{b}_2)^{\frac{1}{2}} } \left( 1 - e^{- \kappa_2(\epsilon_2) (b_2 + \bar{b}_2)^{\frac{1}{2}} }\right)\nonumber \\
		&\leq& \frac{\kappa_1(\epsilon_2)}{\sqrt {b_2 + \bar{b}_2} } \nonumber
	\end{IEEEeqnarray} 
	...
	% the term I_2
	Now, we look the term $I_2$. Since $s \mapsto u_s^{\bar{b}_2}$, $s \mapsto w_s^{\bar{b}_2}$ and $s \mapsto \bar{x}_s^{MFG,\bar{b}_2}$ are continuous over $[T/2,T]$, then the functions $\displaystyle t \mapsto \int_0^t u^{\bar{b}_2}_s ds$ and $\displaystyle t \mapsto \int_0^t (u^{\bar{b}_2}_s- w^{\bar{b}_2}_s)^2 (\bar{x}_s^{MFG,\bar{b}_2} )^2 ds$ are continuous over $[T/2,T]$ and differentiable over $(T/2,T)$. Then with the equation (\ref{eq:x_bar_MFG_explicit_new_notation}) we can apply the Mean Value theorem and deduce that there exists a time points $t_1$ depending on $\bar{b}_2$ which satisfy $\frac{T}{2} < t_1 < T$ such that:
	\begin{IEEEeqnarray}{rCl}
		I_2 &=& \frac{1}{2} B^{\bar{b}_2} \int_{\frac{T}{2}}^T (u^{\bar{b}_2}_t- w^{\bar{b}_2}_t)^2 (\bar{x}_t^{MFG, \bar{b}_2} )^2 dt  \nonumber \\
		&=& \frac{1}{2} B^{\bar{b}_2}  (u_{t_1}^{\bar{b}_2}- w_{t_1}^{\bar{b}_2} )^2 (\bar{x}_{t_1}^{MFG,\bar{b}_2} )^2  \cdot \frac{T}{2} \nonumber \\
		&=& \frac{T  B^{\bar{b}_2} }{4} \cdot (u_{t_1}^{\bar{b}_2}- w_{t_1}^{\bar{b}_2} )^2 \cdot \mathbb{E}[\xi]^2 \exp \left( 2 (b_1 + \bar{b}_1) t_1 - 2\int_0^{t_1} B^{\bar{b}_2} u_s^{\bar{b}_2} ds \right) \nonumber \\
		&\leq &  \kappa_3 (u_{t_1}^{\bar{b}_2}- w_{t_1}^{\bar{b}_2} )^2\cdot (b_2 + \bar{b}_2)^2 \exp\left( - 2 B^{\bar{b}_2} \int_0^{\frac{T}{2}} u_s^{\bar{b}_2} ds \right)
		\label{eq:I_2_b2_bar_inequality}
	\end{IEEEeqnarray}
	where   $\displaystyle \kappa_3 =\frac{T \mathbb{E}[\xi]^2 e^{2 (b_1 + \bar{b}_1) T}}{4 (r + \bar{r}(1-\bar{s})^2)}$ is a constant independent of $\bar{b}_2$.
	
		
	We are going to bound that the last term in (\ref{eq:I_2_b2_bar_inequality}). To do so, we recall that from equation (\ref{eq:u_t_lower_bound_b2_bar_infty}), we have for all $t \in [0,T]$ and all $\bar{b}_2 \geq \bar{b}_2^{*,lower}$ the inequality $ \inf_{0 \leq s \leq T} (b_2 + \bar{b}_2)^{3/2} u_s^{\bar{b}_2} \geq m_u(\epsilon_1)$. 
	Let $\epsilon_3>0$ such that $m_u(\epsilon_1) -  \epsilon_3 >0$. Then there exists $\bar{b}_2^{*,2} := \bar{b}_2^{*,2}(\epsilon_1,\epsilon_3) >0 $ so that for all $\bar{b}_2 \geq \bar{b}_2^{*,2}$ we have: 
	$$ 2 \ln(b_2 + \bar{b}_2) - \epsilon_3\kappa_4 \cdot (b_2 + \bar{b}_2)^{\frac{1}{2}} \leq 0.$$ 
	Hence, let $\bar{b}_2^{*,3} = \max \{\bar{b}_2^{*,upper}, \bar{b}_2^{*,lower}, \bar{b}_2^{*,2} \}$ and $\kappa_4 = \frac{T}{r + \bar{r}(1-\bar{s})^2}$, and together with inequality (\ref{eq:u_t_w_t_uniform_bound_b2_bar_infty}) and (\ref{eq:I_2_b2_bar_inequality}), we obtain that for all $\bar{b}_2 \geq \bar{b}_2^{*,3}$:
	\begin{IEEEeqnarray*}{rCl}
		I_2 &\leq& \kappa_3 \cdot 4 M(\epsilon_1)^2 \cdot (b_2 + \bar{b}_2)^2 \exp \left( - \kappa_4 (b_2 + \bar{b}_2)^{\frac{1}{2}} m_u(\epsilon_1) \right) \nonumber \\
		&=& 4\kappa_3 M(\epsilon_1)^2 e^{ 2 \ln(b_2 + \bar{b}_2) - \epsilon_3 \kappa_4 \cdot (b_2 + \bar{b}_2)^{\frac{1}{2}}} \cdot \exp\left(- \kappa_4 (m_u(\epsilon_1) - \epsilon_3) \cdot (b_2 + \bar{b}_2)^{\frac{1}{2}} \right)   \nonumber \\
		&\leq & \kappa_5(\epsilon_1) \exp\left(- \kappa_4 (m_u(\epsilon_1) - \epsilon_3) \cdot (b_2 + \bar{b}_2)^{\frac{1}{2}} \right)
	\end{IEEEeqnarray*}
