...
\begin{remark}
	...
	From equation (\ref{eq:riccati_limiting_u}) and an analogous Riccati equation for $w_t^{\bar{b}_2 \to 0}$, a sufficient condition for the above equality is:
	$$D^{u,\bar{b}_2 \to \infty} = D^w \quad \text{and} \quad B^{\bar{b}_2 \to 0} (D^{u,\bar{b}_2 \to 0} )^2 + 2 A^{u}D^{u,\bar{b}_2 \to 0} - C^{u,\bar{b}_2 \to 0} = B^{\bar{b}_2 \to 0} (D^w)^2 + 2 A^{w} D^w - C^w  = 0.$$
	In other words, the following three equations provide a sufficient condition for $\lim_{\bar{b}_2 \to 0} PoA^{\bar{b}_2 } =1$:
	$$	\frac{r + \bar{r}(1- \bar{s})^2}{r + \bar{r}(1-\bar{s})} = \frac{q_T+\bar{q}_T(1-s_T)^2}{q_T+\bar{q}_T(1-s_T)},
	\qquad
	 \bar{b}_1 \frac{q_T + \bar{q}_T(1-s_T)^2}{q + \bar{q}(1-s)^2} =  \frac{r + \bar{r}(1- \bar{s})^2}{r + \bar{r}(1-\bar{s})} \frac{q + \bar{q}(1-s)}{q + \bar{q}(1-s)^2}-1,
	$$
	and
