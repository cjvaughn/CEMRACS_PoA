...
\begin{proof}
	...
	\end{equation*}
	we have $c_u \neq c_w$. We want to show that $\lim_{b_2 \to \infty}PoA^{b_2}>1$. To do so, we will show that $(b_2 + \bar{b_2}) \Delta SC^{b_2}\geq c_{num}>0$ and $b_2 SC^{MKV,b_2} \leq M_{den}$ for large $b_2$, where $c_{num}$ and $M_{den}$ are two constants independent of $b_2$.
	...
	Finally, together with inequalities (\ref{eq:numerator_ineq_b2_infinity}) and (\ref{eq:demumerator_ineq_b2_infinity}), let $b_2^{*,case 2} := \max \{ \bar{b}_2,  b_2^{*,numerator}, b_2^{*,denumerator} \}$ so that we have for all $b_2 \geq b_2^{*,case 2}$ the inequalities $ b_2 + \bar{b}_2 \leq 2 b_2$ and 
	\begin{equation}
...
\begin{proof}
	...
	&\leq T(c_\eta+1)+\frac{D^\eta}{c_{\delta_\eta^{b_2 \to \infty}}} \ln \left( 5c_\eta+2 \right)=:M.	
	\end{split}
	\end{equation*}
\end{proof}
