...
%	$$
	...
	\begin{equation}
	 t^* := \min\left\{ \frac{T}{2}, \  \frac{1}{\kappa_2 \cdot \left( b_2^{*,I_1} + \bar{b}_2\right) } \ln \left( \frac{1}{\displaystyle 1- \frac{\epsilon}{\kappa_1} } \right) \right\},
	\label{eq:t^*_b2_infty} 
	\end{equation}
	then we are able to deduce from inequality (\ref{eq:I1_ineq_b2_infinity}) that: 	(ToDo: there is a mistake here! This is only true for $I_1^{b_2^{*,I_1}}$, not for $I_1^{b_2}$ for any $b_2> b_2^{*,I_1}$.)
	\begin{equation}
		I_1^{b_2} \leq \kappa_1 \left(1 - e^{- \kappa_2 (b_2^{*,I_1} + \bar{b}_2) t^*}  \right) \leq \epsilon.\\
