...
\begin{theorem}
	...
	\right.
	\end{equation*}
	where $\lambda = \frac{b_2}{b_2 + \bar{b}_2}\cdot \frac{ r + \bar{r}(1-\bar{s})^2 }{r + \bar{r}(1-\bar{s})}$, and in the case when $\bar{b}_1 > 0$, the third condition becomes
...
\begin{proof}
	...
	From equation (\ref{eq:riccati_ut_sol_deriv}) and the Riccati equations (\ref{eq:riccati_ut}) and (\ref{eq:riccati_wt}), it is easy to check that if the conditions in (\ref{eq:PoA=1_condition_b1_bar>0}) and (\ref{eq:PoA=1_condition_b1_bar=0}) are satisfied, we must have $u_t = w_t$ for all $ t\in [0,T]$. Thus, $PoA = 1$.	
...
\begin{proof}
	...
	\begin{IEEEeqnarray}{rCl}
		(b_2 + \bar{b}_2) \Delta SC ^{b_2} &\geq & \frac{b_2 + \bar{b}_2}{2} B \int_0^{\frac{T}{2}} (u_t - w_t)^2 (\bar{x}^{MFG, b_2})^2 dt \nonumber \\
		&\geq& \frac{\left\vert c_u - c_w \right\vert^2 (\mathbb{E}(\xi))^2}{8 (r+ \bar{r}(1-\bar{s})^2 )} \cdot (b_2 + \bar{b}_2) \int_0^{\frac{T}{2}} \exp \left( - \frac{2 (b_2 + \bar{b}_2)}{r +\bar{r}(1-\bar{s})^2} \cdot \left( c_u + \frac{1}{4} \left\vert c_u - c_w \right\vert \right) \cdot t \right) dt \nonumber \\
		&\geq & \frac{\left\vert c_u - c_w \right\vert^2 (\mathbb{E}(\xi))^2}{16 (c_u +  \vert c_u - c_w \vert / 4)} \left( 1- e^{- \frac{T (c_u + \frac{1}{4} \vert c_u - c_w \vert )}{r+\bar{r}(1-\bar{s})^2}\cdot (b_2 + \bar{b}_2)} \right) \nonumber \\
		&\geq & (2 c_{num} ) \cdot \frac{1}{2} = c_{num}
	\label{eq:numerator_ineq_b2_infinity}	
	\end{IEEEeqnarray}
	where $c_{num} =  \frac{\left\vert c_u - c_w \right\vert^2 (\mathbb{E}(\xi))^2}{32 (c_u +  \vert c_u - c_w \vert / 4)}$ and the last inequality is justified by the choice of $b_2 \geq c_3$. \\
	...
	\begin{IEEEeqnarray*}{rCL}
		...
		&\geq & \left[ \eta_T^{b_2} l(T) - \eta_0^{b_2} l(0) - \int_0^T \eta_t^{b_2} \cdot (l(t))' dt \right]  \nonumber \\
		&= & - \int_0^T \eta_t^{b_2} \cdot \left[ l(t)\cdot( - 2 B^{\eta,b_2} \eta_t^{b_2} ) + 1 \right] dt + l(T) D^{\eta} \nonumber \\
	...
	\begin{IEEEeqnarray}{rCl}
		J_1 &\leq& \frac{q+\bar{q}}{2}  \cdot e^{2b_1 T} \int_0^T \left( Var(\xi) \cdot b_2 e^{-\kappa_5 b_2 t} + \frac{\sigma^2}{\kappa_5} \right) dt  \nonumber \\
		&& \quad + \frac{q_T+\bar{q}_T}{2} \cdot e^{2b_1 T} \left( Var(\xi) b_2 e^{-\kappa_5 T \cdot b_2 } + \frac{\sigma^2}{\kappa_5} \right) + \frac{b_2}{b_2 + \bar{b}_2} (\mathbb{E}(\xi))^2 \cdot (b_2 + \bar{b}_2) w_0^{b_2}\nonumber \\
	...
	\begin{IEEEeqnarray*}{rCl}
		J_2 &=& \frac{1}{2 (r + \bar{r}) }\int_0^T \left(b_2 \eta_t^{b_2} \right)^2 \cdot Var(\xi) e^{2b_1 t} \cdot b_2 \exp \left( - 2 B^{\eta,b_2} \int_0^t \eta_s ds \right) dt \nonumber \\
		&& \hspace{10em}  + \frac{\sigma^2}{2} \cdot  b_2 \int_0^T B^{\eta,b_2} (\eta_t^{b_2})^2 \cdot \left( \int_0^t e^{2b_1(t-s) - 2B^{\eta,b_2} \int_s^t \eta_u^{b_2} du} ds \right)  dt .
	\end{IEEEeqnarray*}
	...
	\begin{IEEEeqnarray}{rCl}
		J_2 &\leq& \frac{Var(\xi) e^{2b_1 T} }{2 (r+\bar{r})} \left[ \int_0^{\frac{T}{2}} \left(b_2 \eta_t \right)^2 \cdot b_2 e^{- \kappa_5 b_2 t} dt  +  \int_{\frac{T}{2}}^{T} (\eta_t^{b_2})^2 \cdot b_2^3 \cdot e^{ - 2 B^{\eta, b_2} \int_0^{t} \eta_s^{b_2} ds } dt \right]  + \frac{\sigma^2  e^{2 b_1 T}}{2} \int_0^T b_2 \eta_t^{b_2} dt  \nonumber \\
		&\leq& 	\kappa_6 \left[ \left(\frac{3 c_\eta}{2} \right)^2 \frac{1}{\kappa_5}(1- e^{-\kappa_5 b_2 \frac{T}{2}}) + M^2 \cdot b_2^3 e^{- \kappa_5 b_2 \frac{T}{2}} \cdot \frac{T}{2} \right] + \frac{\sigma^2  e^{2 b_1 T}}{2} M_{intgl} \nonumber \\
		&\leq& \frac{\kappa_6}{\kappa_5} \left( \frac{9}{4} c_\eta^2 + \frac{M^2 T}{2} \right) + \frac{\sigma^2  e^{2 b_1 T}  M_{intgl}}{2} =: M_{J_2}
	\label{eq:J2_upper_bound_b2_infinity}
	\end{IEEEeqnarray} 
	
	Now, from inequalities (\ref{eq:J1_upper_bound_b2_infinity}) and (\ref{eq:J2_upper_bound_b2_infinity}), and the fact that $M_{J_1}$ and $M_{J_2}$ are independent of $b_2$, equation (\ref{eq:J1_plus_J2_b2_infinity}) gives raise to an upper bound: for all $b_2 \geq b_2^{*,denominator} := \max \{ b_2^{*,J_1}, b_2^{*,J_2} \}$:
	\begin{equation} 
	...
	Finally, together with inequalities (\ref{eq:numerator_ineq_b2_infinity}) and (\ref{eq:demumerator_ineq_b2_infinity}), let $b_2^{*,case 2} := \max \{ \bar{b}_2,  b_2^{*,numerator}, b_2^{*,denominator} \}$ so that we have for all $b_2 \geq b_2^{*,case 2}$ the inequalities $ b_2 + \bar{b}_2 \leq 2 b_2$ and 
	\begin{equation}
		\frac{\Delta SC^{b_2}}{SC^{MKV,b_2}} = \frac{(b_2 + \bar{b}_2) \Delta SC^{b_2} }{\left(\frac{b_2 + \bar{b}_2}{b_2} \right) \cdot (b_2 \cdot SC^{MKV,b_2} ) } \geq \frac{c_{num}}{ 2 M_{den}} > 0.
	\end{equation}
