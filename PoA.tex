...
\end{equation*}
We work over the interval $[0,T]$ limited by a finite time horizon $T \in \mathbb{R}^+$. We assume the drift function $b(t,x,\mu,\alpha,\nu): [0,T] \times \mathbb{R} \times \cP(\mathbb{R}) \times \mathbb{A} \times \cP(\mathbb{A})  \rightarrow \mathbb{R}$ is Lipschitz in each of it's inputs. For the sake of simplicity, we assume that the volatility, $\sigma$, is a positive constant.

\subsubsection*{\textbf{Cost Functionals}}
We assume that we are given two functions $f(t,x,\mu,\alpha, \nu): [0,T] \times \mathbb{R} \times \cP(\mathbb{R}) \times \mathbb{A} \times \cP(\mathbb{A}) \rightarrow \mathbb{R}$ and $g(x,\mu):\mathbb{R}\times \cP(\mathbb{R}) \rightarrow \mathbb{R}$ which we call running and terminal cost functions, respectively. We assume $f$ and $g$ are Lipschitz in each of it's arguments. The goal of player $i$ is to minimize its expected cost as given by:
\begin{equation*}
...
\begin{enumerate}
\item For each feedback function $\phi:[0,T] \times \mathbb{R} \rightarrow \mathbb{R}$, solve the optimal control problem
\begin{equation*}
    \phi^*=\arg \inf_{\phi'}\cS(\phi',\phi).
\end{equation*}
Define the mapping $\Phi:\phi \rightarrow \phi^*$.
...
\begin{equation*}
    \mathcal{N}=\{\phi:[0,T] \times \mathbb{R} \rightarrow \mathbb{R}: \Phi(\phi)=\phi \}.
\end{equation*}
...
\begin{equation*}
\begin{split}
    f(t,x,\mu,\alpha,\nu)&=\frac{1}{2}\left(q(t)x^2+\bar{q}(t)(x-s(t)\bar{\mu})^2 +r(t)\alpha^2+\bar{r}(t)(\alpha-\bar{s}\bar{\nu})^2\right), \\
...
\begin{split}
    ...
    &+\frac{1}{2}\left(q(t)x^2+\bar{q}(t)(x-s(t)\bar{\mu})^2 +r(t)\alpha^2+\bar{r}(t)(\alpha-\bar{s}\bar{\nu})^2\right),
\end{split}
\end{equation*}
...
\end{split}
\end{equation*}
A sufficient condition for optimality is $\partial_{\alpha}H(t,X_t,\bar{\mu}_t,\hat{\alpha}_t,\bar{\nu}_t,y)=0$. According to the Pontryagin stochastic maximum principle, we introduce the function:
\begin{equation*}
    \hat{\alpha}(t,x,\bar{\mu},\bar{\nu},y)=\frac{\bar{r}(t)\bar{s}(t)\bar{\nu}-b_2(t)y}{r(t)+\bar{r}(t)},
\end{equation*}
and use the control $\hat{\alpha}_t=\hat{\alpha}(t,x,\bar{\mu},\bar{\nu},Y_t)$. When solving the fixed point step, we identify $\bar{\nu}_t=\mathbb{E}(\hat{\alpha}_t)$. By taking the expectation, we find:
\begin{equation*}
...
\end{equation*}
So the solution of the mean field game equilibrium problem is given by the solution to the FBSDE system:
\begin{equation}
\begin{split}
...
\begin{split}
    ...
    &+\int_0^T (q_t+\bar{q}_t+(r_t+\bar{r}_t)(a^{MFG}(t)\eta_t^{MFG})^2)v_t^{MFG}\\
    &+(q_t+\bar{q}_t(1-s_t)^2+(r_t+\bar{r}_t(1-\bar{s}_t)^2)(c^{MFG}(t)\bar{\eta}_t^{MFG})^2)(\bar{x}_t^{MFG})^2dt],
\end{split}
\end{equation*}
...
\begin{split}
    ...
    &+\int_0^T (q_t+\bar{q}_t+(r_t+\bar{r}_t)(a^{MKV}(t)\eta_t^{MKV})^2)v_t^{MKV}dt \\
    &+\int_0^T(q_t+\bar{q}_t(1-s_t)^2+(r_t+\bar{r}_t(1-\bar{s}_t)^2)(c^{MKV}(t)\bar{\eta}_t^{MKV})^2)(\bar{x}_t^{MKV})^2dt],
\end{split}
\end{equation*}
where we have used the fact that:
\begin{equation*}
...
We study in the following the variation of $PoA$ by letting one of the coefficients tends to $0$ or to $\infty$. It will be useful for us to note here the scalar Riccati equations associated with $u_t := \lambda \bar{\eta}_t^{MFG}$, $w_t := \bar{\eta}_t^{MKV}$ and $\eta_t$:
    \begin{equation}
...
\begin{corollary}
    Assuming (\ref{eq:assumptions}), if $\bar{b}_1=0$ and $\lambda=1$, then $PoA=1$.
\end{corollary}
\begin{proof}
    Under these assumptions, $(A^u,C^u,D^u)=(A^w,C^w,D^w)$, and thus $(u_t)_{0\leq t\leq T}=(w_t)_{0\leq t\leq T}$. Thus, $\Delta \bar{\eta}_t=0,\ \forall t \in [0,T]$. By Proposition \ref{prop:diff_SC}, $\Delta SC=0$. It can be shown that $w_0\geq0$. Since $v_t>0$ for $t>0$, clearly $SC^{MKV}>0$. Therefore, $PoA=1$.
\end{proof}
...
\begin{proof}
	...
	Moreover, since $\bar{x}_t^{MFG} = \bar{x}_0 \exp(\int_0^t (b_1 + \bar{b}_1 - B\cdot u_s)ds)$ and $v_t = Var(\xi)\exp(2\int_0^t (b_1 - B^\eta \eta_s) ds) + \sigma^2 \int_0^t \exp(2 \int_s^t (b_1 - B^\eta \eta_u) du) ds$, we have 
	...
	\end{equation*}
	By their explicit formulas, it can be shown that $w^{\infty,r}_0\geq0$ and $\lim_{r \to \infty}v_t >0$ for $t>0$ and therefore,
	\begin{equation*}
...
\begin{proof}
	...
	By taking the limit $b_2 \to 0$, we have $\delta^\pm_u = -A^u \pm \vert A^u \vert $ and $\delta^{+}_u - \delta^{-}_u = 2 \vert A^u \vert $. Thus, from equation (\ref{eq:riccati_ut_sol}) we deduce
	...
	\begin{IEEEeqnarray*}{rCl}
		\Delta SC &\sim & \frac{1}{2} (\sqrt{C^{\infty,b_2} }- \sqrt{C^w})^2 \int_0^T (\bar{x}_t^{MFG} )^2 dt \\
			&=& \frac{(\sqrt{C^{\infty,b_2} }- \sqrt{C^w})^2 \cdot \bar{x}_0^2 }{2 (2b_1 +2\bar{b}_1 - 2 \kappa_u (b_2 + \bar{b}_2) )} \left(  e^{(2b_1 + 2\bar{b}_1 - 2\kappa_u (b_2+ \bar{b}_2) ) T} - 1\right)\\
			&\sim & \frac{\zeta_1 }{b_2 } e^{- 2 \kappa_u T \cdot b_2 }  + \frac{\zeta_2}{ b_2},
	\end{IEEEeqnarray*}
	and using equation (\ref{eq:SC_MFG_MKV_2}), we deduce:
	\begin{IEEEeqnarray*}{rCl}
		SC^{MKV} &\sim & (q + \bar{q} )  \int_0^T v_t dt + \frac{1}{2}(q_T + \bar{q}_T)v_T + \frac{1}{2}w_0 \bar{x}_0^2\\
		&\sim& (q + \bar{q}) \left[ \left(\frac{Var(\xi)}{2b_1 - 2\kappa_v b_2} + \frac{\sigma^2}{(2b_1 - 2\kappa_v b_2)^2} \right) e^{2b_1 T - 2 \kappa_v T b_2} - \left( \frac{Var(\xi) + \sigma^2 T}{2b_1 - 2\kappa_v b_2} + \frac{\sigma^2}{(2b_1 - 2 \kappa_v b_2)^2} \right) \right]\\
		& & + \frac{(q_T + \bar{q}_T)}{2} \left[  Var(\xi) e^{2b_1T - 2\kappa_v T\cdot b_2} + \frac{\sigma^2}{2b_1 - 2\kappa_v b_2} \left(e^{2b_1 T - 2\kappa_v T\cdot b_2} - 1\right) \right] + \frac{\bar{x}_0^2 \sqrt{(r+ \bar{r}(1-\bar{s})^2) C^w} }{2(b_2 + \bar{b}_2)}\\
		&\sim & \left( \frac{\zeta_3}{b_2^2} + \frac{\zeta_4}{b_2} + \zeta_5 \right) e^{-2\kappa_v T\cdot b_2}  + \left( \frac{\zeta_6}{b_2^2} + \frac{\zeta_7}{b_2}  \right)
	\end{IEEEeqnarray*}
...
and thus,
\begin{equation*}
    \bar{x}_t=\bar{x}_0 e^{\int_0^t(a^x_u+a^{\bar{x}}_u+(a^y_u+a^{\bar{y}}_u)\bar{\eta}_u)du}.
\end{equation*}
