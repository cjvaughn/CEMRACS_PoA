...
\begin{proof}
	...
	\end{split}
	\end{equation*}
	It is easy to show that for every time $t\in [0,T]$, $\lim_{r \to \infty}u^r_t=u^{r \to \infty}_t$. Next, our goal is to bound the $u^r_t$ uniformly over $t \in [0,T]$ for large enough $r$.
	
	Note that $B^r, C^{u,r}, \lambda^{r\to \infty}, C^{r \to \infty}_u>0$, $D^{u,r} , D^{r \to \infty}_u\geq 0$, and $\delta^{-,r}_u < 0 < \delta^{+,r}_u$. Let $\epsilon>0$. Then there exists $r^*>0$ such that for $r \geq r^*$, we have $\max\{ B^{r}, C^{u,r}, D^{u,r} \} \leq \max\{C^{r \to \infty}_u,D^{r \to \infty}_u \}+\epsilon=: \zeta$. When $A^u \neq 0$, we have the following two inequalities for $r \geq r^*$:
	...
	\begin{IEEEeqnarray*}{rl}
		...
		&\geq \delta^{+,r}_u e^{- (\delta^{+,r}_u - \delta^{-,r}_u)(T-t)} - \delta^{-,r}_u \\		
