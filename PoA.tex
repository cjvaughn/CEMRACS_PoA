...
\begin{proof}
	...
	Note that  $A^u \leq 0$, $B^r, C^{u,r}, \lambda^{r\to \infty}, C^{r \to \infty}_u>0$, $D^{u,r} , D^{r \to \infty}_u\geq 0$, and $\delta^{-,r}_u < 0 < \delta^{+,r}_u$. Let $\epsilon>0$. Then there exists $r^*>0$ such that for $r \geq r^*$, we have $\max\{ B^{r}, C^{u,r}, D^{u,r} \} \leq \max\{C^{r \to \infty}_u,D^{r \to \infty}_u \}+\epsilon=: \zeta$. When $A^u \neq 0$, we have the following two inequalities for $r \geq r^*$ and $\forall t \in [0,T]$:
	...
	\begin{IEEEeqnarray*}{rl}
		...
		&\geq(-A^u + \vert A_u \vert) e^{-2T \sqrt{(A^u)^2 + \zeta^2}}\\
		&\geq -2A^u e^{-2T \sqrt{(A^u)^2 + \zeta^2} }\\
		&>  0.
	\end{IEEEeqnarray*}
	Thus, for $A^u \neq 0$, we can deduce that for $r>r^*$, for every $t \in [0,T]$:
	...
	$$ \vert \bar{x}^{r,MFG}_t \vert \leq \vert \mathbb{E}(\xi) \vert e^{ (b_1 + \bar{b}_1 + \zeta \kappa ) T }, \quad t \in [0,T]. $$  
	
	Similarly, the solutions $w^r_t$ and $\eta^r_t$ of equations (\ref{eq:riccati_wt}) and (\ref{eq:riccati_etat}) converge for each time $t\in [0,T]$ towards the solutions $w^{r\to \infty}_t$ and $\eta_t^{r\to \infty}$ of some first-order differential equations. Namely, for every $t\in [0,T]$,
	$$w^r_t \xrightarrow[r \to \infty]{} w_t^{r\to \infty},\qquad \eta^r_t \xrightarrow[r \to \infty]{} \eta_t^{r\to \infty}$$
	and the functions $w^{r}$ and $\eta^{r}$ are uniformly bounded over $t \in[0,T]$ and large $r$.
	...
	and thus,
	\begin{equation*}
	\lim_{r \to \infty} \Delta SC^r = \lim_{r \to \infty} \frac{1}{2}\cdot \frac{(b_2 + \bar{b}_2)^2}{r + \bar{r}(1 - \bar{s})^2}\int_0^T (u^r_t - w^r_t)^2 ( \bar{x}_t^{r,MFG})^2 dt = 0.
	\end{equation*}
