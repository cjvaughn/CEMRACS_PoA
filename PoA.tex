...
In the following propositions, based on assumption (\ref{eq:assumptions}) and the positivity of coefficients, we assume in addition that $b_1 > 0, D^{u} > 0, D^w > 0, D^{\eta} >0$ and the initial condition $\xi$ satisfies $\mathbb{E}(\xi) \neq 0$. These assumptions allow us to better understand the main ideas and techniques employed in the following propositions.


% proposition r to infty
\begin{proposition}
	Assuming (\ref{eq:assumptions}), then:
	\begin{equation*}
...
\begin{proof}
	...
	\end{equation*}
	It is easy to show that for every time $t\in [0,T]$, $\lim_{r \to \infty}u^r_t=u^{r \to \infty}_t$, and thus, $\lim_{r \to \infty}B^ru^r_t=0$. 
	
	
%	Note that $D^{r \to \infty}_u > 0$ and $\lim_{r \to \infty} B^{r}(D^{r,u})^2 + 2 A^u D^{u,r} - C^{u,r} = \infty$, from equation (\ref{eq:riccati_ut_sol_deriv}) we know that for all large $r$, the function $ t \mapsto u_t^{r}$ is increasing. Moreover, there exists $r^*>0$ such that for all $r \geq r^*$ and all $t \in [0,T]$,
%	$$ 0 \leq u_t^{r} \leq u_T^{r}  = D^{u,r} \leq D^{r\to \infty}_u + 1.$$
	Next, our goal is to bound the $u^r_t$ uniformly over $t \in [0,T]$ for large enough $r$.
	Note that  $A^u < 0$, $B^r$, $C^{u,r}$, $\lambda^{r\to \infty}$, $C^{r \to \infty}_u>0$, $D^{u,r}$, $D^{r \to \infty}_u\geq 0$, and $\delta^{-,r}_u < 0 < \delta^{+,r}_u$. Let $\epsilon>0$. Then there exists $r^*>0$ such that $\max_{r > r^*}\{ B^{r}, C^{u,r}, D^{u,r} \} \leq \max\{C^{r \to \infty}_u,D^{r \to \infty}_u \}+\epsilon=: c_1$. Thus, we can deduce that for $r>r^*$, for every $t \in [0,T]$:
	\begin{equation}
	...
	\end{equation*}
	Moreover, $\bar{x}_t^{MFG,r}$ is uniformly bounded for $t \in [0,T]$, which comes from the fact that $u^r_t \geq 0$ for all $t \in [0,T]$ so that,
	...
	\end{equation*}
	Therefore, from Proposition \ref{prop:diff_SC} and equation (\ref{eq:PoA_LQ_new_notation}), we have 
	$$\displaystyle \lim_{r \to \infty} PoA^r = 1.$$
	
	Idem, by replacing $\lambda^{r \to \infty}$ with $\lambda^{\bar{r} \to \infty}: = \frac{b_2}{b_2 + \bar{b}_2} (1 - \bar{s})$, we obtain $\displaystyle \lim_{\bar{r} \to \infty} PoA^{\bar{r}} = 1$.
\end{proof}


% proposition b_2 to \infty
\begin{proposition}
	Assuming (\ref{eq:assumptions}), and if
	\begin{equation*}
...
\begin{proof}
	...
	From the assumption $q_T+\bar{q}_T(1-s_T)>0$, we have $D^{u,b_2}>0$. From equation (\ref{eq:riccati_ut_sol}), for each $t \in[0,T)$, we deduce:
	...
	$\lim_{b_2 \to \infty} B^{b_2} (D^{u,b_2})^2 + 2 A^u D^{u,b_2} - C^{u,b_2} = \infty,$ there exists $b_2^{*,u,upper} >0$ such that for all $b_2 \geq b_2^{*,u,upper}$, we have $t \mapsto u_t^{b_2}$ is increasing and 
	...
	\end{equation}
	and the functions $t \mapsto u_t^{b_2}$, $t \mapsto w_t^{b_2}$ and $t \mapsto \eta_t^{b_2}$ are increasing on $[0,T]$. 
	...
	We would like to show in this case that $\lim_{b_2 \to \infty} \frac{\Delta SC}{SC^{MKV}} = 0$. Our approach is to split the interval $[0,T]$ into two parts: $[0,T/2]$ and $[T/2,T]$.	
	...
	\begin{IEEEeqnarray}{rCl}
		I_2^{b_2} &\leq&  \frac{(b_2+ \bar{b}_2)^3}{r + \bar{r}(1- \bar{s})^2} \cdot 4M^2 \cdot e^{2(b_1 + \bar{b}_1) T} \int_{\frac{T}{2}}^{T} \exp\left( - \frac{T(b_2 + \bar{b}_2)}{r + \bar{r}(1-\bar{s})^2} \cdot m_u \right) dt \nonumber\\	
	...
	Let $b_2^* := \max \{ b_2^{*,I_1}, b_2^{*,I_2}\}$. Then equations (\ref{eq:I_1_plus_I_2}), (\ref{eq:I1_ineq_b2_infinity}) and (\ref{eq:I2_b2_infty}) give:
	\begin{equation*}
...
%proposition b_2 \to 0
\begin{proposition}
	Assume (\ref{eq:assumptions}). If $\bar{b}_2= 0$, then:
	\begin{equation*}
	\lim_{b_2 \to 0} PoA = 1,
	\end{equation*}
...
\begin{proof}
	...
	\begin{equation*}
	 \vert u_t^{b_2} \vert \leq \frac{c_1 + 2 \epsilon \sqrt{(A^u)^2 + c_1^2} }{- 2 A^u e^{-2T \sqrt{(A^u)^2 + c_1^2} } } ,\qquad \forall  \  t \in [0,T]
	\end{equation*}
	...
	and thus,
	$$ 0 < \lim_{b_2 \to 0} SC^{MKV,b_2} < \infty.$$
	Therefore, combined with equation (\ref{eq:PoA_LQ_new_notation}), we conclude that $$\lim_{b_2 \to 0} PoA^{b_2} = 1 + \frac{\displaystyle \lim_{b_2 \to 0}\Delta SC^{b_2}}{\displaystyle \lim_{b_2 \to 0} SC^{MKV,b_2} } =: PoA^{b_2 \to 0} > 1.$$
...
\begin{proposition}
	...
	If	$	\frac{r + \bar{r}(1- \bar{s})^2}{r + \bar{r}(1-\bar{s})} \neq \frac{q_T+\bar{q}_T(1-s_T)^2}{q_T+\bar{q}_T(1-s_T)}$ then:
	\begin{equation*}
	\lim_{\bar{b}_2 \to 0} PoA =: PoA^{\bar{b}_2\to 0} > 1. 
	\end{equation*}	
	When $\bar{b}_2$ tends to infinity, we always have:
	$$ \qquad\lim_{\bar{b}_2 \to \infty} PoA = 1.$$
	\label{prop:b2bar}
\end{proposition}

\begin{proof}	
	\textbf{Case 1: } When $\bar{b}_2 \to 0$, we have:
	\begin{equation*}
	\begin{array}{l}
	...
	Moreover, $\bar{x}^{MFG,\bar{b}_2}$ is uniformly bounded for all $\bar{b}_2<\bar{b}^*_2$ and for all $t \in [0,T]$. From the non-negativity of $u_t$, we have:
	...
	\label{eq:delta_SC_limit_b2_bar_0}
	\end{equation}	
	...
	\textbf{Case 2:} Now, for the case when $\bar{b}_2 \to \infty$,  we have:
	\begin{equation*}
	\begin{array}{l}
		\lambda^{\bar{b}_2} \to 0,\\
	...
	\begin{equation*}
	\begin{array}{l}
	(b_2 + \bar{b}_2)^{\frac{3}{2}}\cdot u^{\bar{b}_2}_t \xrightarrow[\bar{b}_2 \to \infty]{} (r + \bar{r}(1-\bar{s})^2) \cdot c_{\delta^{\bar{b}_2 \to \infty}_u} =: c_u, \\
	(b_2 + \bar{b}_2) w_t^{\bar{b}_2} \xrightarrow[\bar{b}_2 \to \infty]{} (r + \bar{r}(1-\bar{s}^2) ) \cdot c_{\delta_w^{\bar{b}_2\to \infty}} =: c_w.
	\end{array}
	\end{equation*}
	
	Next, we provide a uniform upper bound for $u_t^{\bar{b}_2}$ over $[0,T]$ and large $\bar{b}_2$. Let $\zeta_1 = \frac{1}{2} \min \left\{ c_{\delta_u^{\bar{b}_2 \to \infty}}, \  c_{\delta_w^{\bar{b}_2 \to \infty} } \right\}$. Then there exists $\bar{b}_2^{*,u} >0 $ such that for all $\bar{b}_2 \geq \bar{b}_2^{*,u}$,
	...
	\begin{equation}
		...
		\leq  \frac{\zeta_1 + \zeta_1 (2 c_{\delta_u^{\bar{b}_2 \to \infty}} + 2 \zeta_1)}{ c_{\delta_u^{\bar{b}_2 \to \infty}} - \zeta_1} \leq 3 c_{\delta_u^{\bar{b}_2 \to \infty}} + 1
	...
	Fix $\epsilon>0$. In the following, we will show that $\Delta SC^{\bar{b}_2} = I^{\bar{b}_2}_1 + I^{\bar{b}_2}_2 \leq 2\epsilon$ for large $\bar{b}_2$. First, consider $I^{\bar{b}_2}_1$. Let $\zeta_2 = c_u / 2$. Because $ \lim_{\bar{b}_2 \to \infty} B^{\bar{b}_2} (D^{u,\bar{b}_2})^2+ 2 A^{u} D^{u,\bar{b}_2} - C^{u,\bar{b}_2} >0$, there exists $\bar{b}_2^{*,0} \geq \bar{b}_2^{*,upper}$ so that for all $\bar{b}_2 \geq \bar{b}_2^{*,0}$, the functions $s \mapsto u_s^{\bar{b}_2}$ and $s \mapsto w_s^{\bar{b}_2}$ are increasing, and for all $s \in [0, T/2]$:
	\begin{IEEEeqnarray*}{c}
		 ...
		 \left\vert (b_2 + \bar{b}_2) u_s^{\bar{b}_2} \right\vert \leq \left\vert (b_2 + \bar{b}_2) u_{\frac{T}{2}}^{\bar{b}_2} \right\vert \leq  \zeta_2, \quad \text{and} \quad 
	...
	\end{IEEEeqnarray} 
	where $\kappa_1 = \frac{\mathbb{E}(\xi)^2 [\zeta_2^2 + (c_w + \zeta_2)^2 ] e^{2(b_1 + \bar{b}_1) \frac{T}{2}} }{2 (c_u - \zeta_2)}$ and $ \kappa_2 = \frac{(c_u - \zeta_2)T}{r+ \bar{r}(1-\bar{s})^2}$ are independent of $\bar{b}_2$. Therefore, there exists $\bar{b}_2^{*,I_1} \geq \bar{b}_2^{*,0}$ such that for $\bar{b}_2 \geq \bar{b}_2^{*,I_1}$, we have $I_1^{\bar{b}_1} \leq \epsilon$.
	...
	\end{equation*}
	Moreover, recall that $\eta_t$ and $v_t$ are invariant with respect to $\bar{b}_2$ and $0<v_t<\infty$ for $t>0$. Clearly we also have $w^{\bar{b}_2}_0 \geq 0$ and $w^{\bar{b}_2}_0 \xrightarrow[\bar{b}_2 \to \infty]{} 0$. Thus, we obtain:
...
\begin{remark}
	In the case when $\bar{b}_2$ tends to zero, if $\lim_{\bar{b}_2 \to 0} PoA^{\bar{b}_2 } =1$, then since $0< SC^{MKV, \bar{b}_2 \to 0} < \infty$ we must have $\lim_{\bar{b}_2 \to 0} \Delta SC^{\bar{b}_2} = 0$. Hence, from equation (\ref{eq:delta_SC_limit_b2_bar_0}) we  can derive that that for $t \in [0,T]$,
	$$u_t^{\bar{b}_2 \to 0} = w_t^{\bar{b}_2 \to 0}.$$
	We know that $u_t^{\bar{b}_2 \to 0}$ and $w_t^{\bar{b}_2 \to 0}$ are solutions to some Riccati equations, the above equality implies $ D^{u,\bar{b}_2 \to \infty} =  u_T^{\bar{b}_2 \to 0} = w_T^{\bar{b}_2 \to 0} = D^w$ and 
	$$ B^{\bar{b}_2 \to 0} (D^{u,\bar{b}_2 \to 0} )^2 + 2 A^{u}D^{u,\bar{b}_2 \to 0} - C^{u,\bar{b}_2 \to 0} = B^{\bar{b}_2 \to 0} (D^w)^2 + 2 A^{w} D^w - C^w  = 0.$$
	In another words, the coefficients have to satisfy the following three equations:
	$$	\frac{r + \bar{r}(1- \bar{s})^2}{r + \bar{r}(1-\bar{s})} = \frac{q_T+\bar{q}_T(1-s_T)^2}{q_T+\bar{q}_T(1-s_T)},
	\qquad
	 \bar{b}_1 \frac{q_T + \bar{q}_T(1-s_T)^2}{q + \bar{q}(1-s)^2} = 1 - \frac{r + \bar{r}(1- \bar{s})^2}{r + \bar{r}(1-\bar{s})} \frac{q + \bar{q}(1-s)}{q + \bar{q}(1-s)^2},
	$$
	and
	$$ \frac{b_2^2}{r + \bar{r}(1-\bar{s})^2} \left(q_T + \bar{q}_T(1- s_T)^2 \right)^2 - 2 (b_1 + \bar{b}_1) \left(q_T + \bar{q}_T(1- s_T)^2 \right) - \left(q_ + \bar{q}(1- s)^2 \right) = 0.$$\\
\end{remark}
...
\begin{proposition}\label{prop:b1_b1bar}
	...
	\begin{equation*}
	\begin{split}
	    \lim_{b_1 \to 0} PoA =: PoA^{b_1 \to 0} > 1\qquad &\text{and} \qquad \lim_{\bar{b}_1 \to 0} PoA =: PoA^{\bar{b}_1 \to 0} > 1,\\
...
\begin{proof}
	...
	\end{equation*}
	Moreover, $\bar{x}_t^{MFG,b_1}$ is uniformly bounded over $b_1<b_1^*$ and $t \in [0,T]$, i.e. 
	...
	$\lim_{b_1 \to \infty}B (D^{u})^2 + 2 A^{u,b_1} D^{u} - C^{u} = \lim_{b_1 \to \infty}B (D^{w})^2 + 2 A^{w,b_1} D^{w} - C^{w}=-\infty$. Denote $h_u(b_1,t) = B \frac{u_t^{b_1}}{b_1}$ and $h_w(b_1,t) = B \frac{w_t^{b_1}}{b_1}$. We have the following limits, for all $t \in [0,T)$:
	\begin{equation}
		\lim_{b_1 \to \infty} h_u(b_1,t) =  \lim_{b_1 \to \infty} h_w(b_1,t) = 2.
	\label{eq:ut_wt_lim_b1_infty}
	\end{equation}
	From equation (\ref{eq:riccati_ut_sol_deriv}), and together with equation (\ref{eq:ut_wt_lim_b1_infty}), there exists $b_1^{*,upper} >0$ such that for all $b_1 \geq b_1^{*,upper}$, the functions $t \mapsto u_t^{b_1}$ and $t \mapsto w_t^{b_1}$ are decreasing and
	$$ \sup_{0 \leq t \leq T} \left\{ h_u(b_1, t), h_w(b_1,t)\right\} \leq \max \left\{ h_u(b_1, 0), h_w(b_1,0) \right\} \leq 2+1= 3.$$
	...
	We adapt the methodology in Proposition \ref{prop:b2_to_infty} and split the interval $[0,T]$ into two parts: $[0, 3T/4]$ and $[3T/4, T]$. For all $b_1 \geq b_1^{*,I_1}$, similar to equation (\ref{eq:I_1_plus_I_2}), we have:
	\begin{equation*}
		\frac{\Delta SC^{b_1}}{SC^{MKV,b_1}} \leq \frac{I^{b_1}_1 + I^{b_1}_2 }{h_w(b_1,0)},
	\end{equation*}
	where
	\begin{IEEEeqnarray*}{rCl}
	I^{b_1}_1 &=& b_1 \int_0^{\frac{3}{4}T} \left( h_u(b_1,t) - h_w(b_1,t) \right)^2 e^{2\bar{b}_1 t} \cdot \exp \left( 2b_1 t - 2 b_1 \int_0^t h_u(b_1,s) ds \right) dt \nonumber \\
	...
	\begin{IEEEeqnarray*}{rCl}
		 I^{b_1}_2 &=& b_1 \int_{\frac{3}{4}T}^T \left( h_u(b_1,t) - h_w(b_1,t) \right)^2 e^{2\bar{b}_1 t} \cdot \exp \left( 2b_1 t - 2 b_1 \int_0^t h_u(b_1,s) ds \right) dt \nonumber \\
	...
	$$\max \left\{ \left|\frac{u^{\bar{b}_1}_0}{\bar{b}_1}-\frac{1}{B} \right|,\left|\frac{w^{\bar{b}_1}_{T/2}}{\bar{b}_1}-\frac{2}{B} \right| \right\} <\frac{1}{4B},$$
	and the functions $t \mapsto u_t^{\bar{b}_1}$ and $ t \mapsto w_t^{\bar{b}_1}$ are decreasing. Thus, for all $t \in [0, T/2]$ we have:
	\begin{equation*}
	...
	Since $u^{\bar{b}_1}_t$ is decreasing for $\bar{b}_1> \bar{b}_1^{*,lower}$, we have $\bar{b}_1-B u_t^{\bar{b}_1} \geq \bar{b}_1-B u_0^{\bar{b}_1}$. We have the following limits for $t\in[0,T)$:
	\begin{equation*}
	\begin{split}
	...
	Since $\lim_{\bar{b}_1 \to \infty}(\bar{b}_1-B u_0^{\bar{b}_1}) = -2b_1 < 0$, there exists $\bar{b}_1^{*}\geq\bar{b}_1^{*,lower}$, such that for $\bar{b}_1 \geq \bar{b}_1^{*}$, $(\bar{b}_1-B u_0^{\bar{b}_1}) \geq -3b_1$. Returning to equation (\ref{eq:prop_7_case_4_numerator}), and using equation (\ref{eq:lower_bound_u_w}) we have for $\bar{b}_1 \geq \bar{b}^*_1$:
	\begin{equation}
