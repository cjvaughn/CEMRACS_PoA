...
\begin{equation}
PoA =1+ \frac{\Delta SC}{SC^{MKV}}= 1 +  \frac{ \displaystyle   \frac{(b_2 + \bar{b}_2)^2}{r + \bar{r}(1-\bar{s})^2}  \int_0^T (u_t-w_t)^2 \cdot (\bar{x}_t^{MFG})^2 dt }{\displaystyle \int_0^T \left[q+\bar{q} + \frac{b_2^2}{r+\bar{r}} \eta_t^2 \right] v_t dt + (q_T + \bar{q}_T) v_T + w_0 (\mathbb{E}(\xi))^2}.
...
\end{proposition}
\begin{proof}
	First, we consider $r \to \infty$. When assumption $(\ref{eq:assumptions})$ is satisfied, for every given $r >0$, we have existence and uniqueness of the solutions $u^{r}_t$, $w^{r}_t$ and $\eta^{r}_t$ to the scalar Riccati equations (\ref{eq:riccati_ut})-(\ref{eq:riccati_etat}). Note that we have added the superscript $r$ to emphasize the dependence on this coefficient.
	...
	In order to show that $\lim_{r \to \infty} PoA^r = 1$, it suffices to show that $\lim_{r \to \infty}\Delta SC^{r} = 0$ while $\lim_{r \to \infty} SC^{r, MKV} >0$. From equation (\ref{eq:PoA_LQ_new_notation}), a sufficient condition is to demonstrate that $\lim_{r \to \infty}\int_0^T (u^r_t - w^r_t)^2 (\bar{x}_t^{r,MFG} )^2 dt<\infty$, $\lim_{r \to \infty}v^r_T>0$, and $\lim_{r \to \infty}w^r_0>0$.\\

	% modification Main point 5 (nicolas)
	Let $u^{r \to \infty}:[0,T] \to \mathbb{R}$ be the solution to the first-order differential equation:
	\begin{equation}
	...
	\end{split}
	\end{equation*}
	It is easy to show that for every time $t\in [0,T]$, $\lim_{r \to \infty}u^r_t=u^{r \to \infty}_t$.
