...
\begin{proof}
	...
	Next, we derive a strictly positive uniform lower bound for $(b_2 + \bar{b}_2) u_t^{b_2}$ over $[0,T]$ and large $b_2$. Let $\zeta_1 =\frac{1}{2} \min \left\{ c_{\delta_u^{b_2 \to \infty}}, D^{u,b_2 \to \infty} \right\}$. Then there exists $b_2^{*,lower}>0$ such that for all $b_2 \geq b_2^{*,lower}$,
	$$ 
	...
	\end{IEEEeqnarray}
	Idem, we can have a strictly positive uniform lower bound for $b_2 \eta_t^{b_2}$ over $[0,T]$ and large $b_2$. Let $b^{*,\eta,lower}_2 >0$ such that for all $b_2 \geq b^{*,\eta,lower}$,
	\begin{equation}
		b_2 \eta_t^{b_2} \geq m_\eta > 0.
	...
	We would like to show that $\lim_{b_2 \to \infty} \frac{\Delta SC}{SC^{MKV}} = 0$. Our approach is to split the interval $[0,T]$ into two parts: $[0,\frac{T}{2}]$ and $[\frac{T}{2},T]$.	
	...
	$$ \zeta_2 =\min \{\frac{1}{2}\sqrt{\epsilon c}e^{-T(b_1+\bar{b}_2)},\frac{c}{2}\}.$$
...
%	\end{equation}	
	Since $u_t^{b_2}$ is positive over $[0,T]$, we know from equations (\ref{eq:u_t_bound}) and (\ref{eq:w_t_bound}) that for all $b_2 \geq b_2^{*,upper}>b_2^{*,lower}$ and all $t \in [\frac{T}{2},T]$:
	\begin{equation*}
	...
	\begin{IEEEeqnarray*}{rCl}
		...
		&=& \kappa_3 (b_2 + \bar{b}_2)^3 e^{- \kappa_4 (b_2 + \bar{b}_2)} \xrightarrow[b_2 \to \infty]{} 0,
	\end{IEEEeqnarray*}
	where $\kappa_3 = \frac{4 TM^2 e^{2(b_1 + \bar{b}_1) T} }{r + \bar{r}(1-\bar{s})^2} >0$ and $\kappa_4 :=  \frac{T  m_u}{r + \bar{r}(1-\bar{s})^2}>0$ are constants independent of $b_2$. 	
	Hence, there exists $b_2^{*,I_2} > b_2^{*,upper}$ such that for all $b_2 \geq b_2^{*,I_2}$: 
	\begin{equation}
		I_2^{b_2} \leq \epsilon.
