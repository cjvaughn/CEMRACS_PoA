...
\begin{proof}
	...
	\begin{IEEEeqnarray}{rCl}
		...
		&=&  \kappa_1 \left[ (b_2 + \bar{b}_2) (u_{t_1}^{b_2}- w_{t_1}^{b_2} ) \right]^2\cdot  \exp \left( - 2 B^{b_2} u_{t_2}^{b_2} \cdot t_1 \right) \nonumber \\
		&= & \kappa_1 \left[ h_u(b_2,t_1) - h_w(b_2,t_1) \right]^2 \cdot  e^{- \kappa_2 (b_2 + \bar{b}_2) h_u(b_2,t_2)},
	...
	\begin{IEEEeqnarray}{rCl}
		\frac{\Delta SC}{SC^{MKV}} &=& \frac{\displaystyle \kappa_1 \left[ h_u(b_2,t_1) - h_w(b_2,t_1) \right]^2 \cdot  e^{- \kappa_2 (b_2 + \bar{b}_2) h_u(b_2,t_2)} }{\displaystyle \left( (q + \bar{q})T + \frac{T}{r + \bar{r}} h_\eta(b_2,t_3)^2 \right) v_{t_3}^{b_2} + (q_T + \bar{q}_T) v_T^{b_2} + \frac{1}{b_2+\bar{b}_2} (\mathbb{E}(\xi) )^2 h_w(b_2,0)  } \nonumber \\
		&=& \frac{l_{u,v}(b_2) \cdot (b_2 + \bar{b}_2) e^{- \kappa_2 (b_2 + \bar{b}_2) h_u(b_2,t_2)}} { l_\eta(b_2) \cdot b_2 v_{t_3}^{b_2}  + l_1(b_2) \cdot b_2 v_T^{b_2} + (\mathbb{E}(\xi))^2 h_w(b_2,0) }
	\label{eq:delta_SC_SC^MKV_b_2_infinity}
	\end{IEEEeqnarray}
	where 
	$\displaystyle l_{u,v}(b_2) = \kappa_1 \left[ h_u(b_2,t_1) - h_w(b_2,t_1) \right]^2 , \  l_\eta(b_2) =  \left( (q + \bar{q})T + \frac{T}{r + \bar{r}} h_\eta(b_2,t_3)^2 \right) \frac{b_2 + \bar{b}_2}{b_2}$
	and $\displaystyle l_1(b_2) = (q_T + \bar{q}_T) \frac{b_2 + \bar{b}_2}{b_2}$.	
	...
	Finally, we will show that for any fixed $t \in [0,T)$ and $\kappa >0$, we have
	\begin{equation}
		(b_2 + \bar{b}_2) e^{- \kappa (b_2 + \bar{b}_2) h_u(b_2,t)} \xrightarrow[b_2 \to \infty]{} 0 \qquad \text{and} \qquad b_2 e^{-\kappa b_2 h_\eta(b_2, t)} \xrightarrow[b_2 \to \infty]{} 0.
	\label{eq:b_2_vt_limit}
	\end{equation}
	Therefore, the numerator of equation (\ref{eq:delta_SC_SC^MKV_b_2_infinity}) tends towards $0$ whilst the denominator tends towards $(\mathbb{E}(\xi))^2 c_4>0$ because equation (\ref{eq:vt_b_2_infinity}) gives raise to
	$$ b_2 v_T^{b_2} = \kappa_3 \cdot b_2 e^{-\kappa_4 b_2 h_\eta(b_2,t_4)} + \kappa_5 \cdot b_2 e^{-\kappa_6 b_2 h_\eta(b_2,t_6)} \xrightarrow[b_2 \to \infty]{} 0,$$
	and so $b_2 v_{t_3}^{b_2} \xrightarrow[b_2 \to \infty]{} 0$.
	Together with the limits of $l_{u,v}(b_2), l_\eta(b_2)$ and $l_1(b_2)$, we have
...
\begin{proof}
	...
	Similar to the case in Proposition \ref{prop:r_bar_r}, we can derive an uniform bound for $u_t^{b_2}$ over $[0,T]$ for any $b_2$ small enough. Indeed, for any fixed $\epsilon >0$ there exits $b_2^* >0$ such that for any $b_2 \leq b_2^*$, $\max \{B^{b_2}, C^{u,b_2}, D^{u,b_2} \} \leq B^{b_2 \to \infty} + \epsilon =: c_1$ and
	\begin{equation*}
...
\begin{proof}
	...
	\begin{split}
	 	...
	 	& \xrightarrow[\bar{b}_2 \to \infty]{}  (r + \bar{r}(1-\bar{s})^2) c_{\delta^{\bar{b}_2 \to \infty}_u}=: c_3,
	 \end{split} 
	\end{equation*}
	where $\zeta = \frac{b_2 (r + \bar{r}(1-\bar{s})^2) }{r+ \bar{r}(1-\bar{s})} (q_T + \bar{q}_T(1-s_T))$. 
	
	Since $u_t^{\bar{b}_2}$, $w_t^{\bar{b}_2}$ and $\bar{x}_t^{MFG,\bar{b}_2}$ are continuous over $[0,T]$, then the functions $t \mapsto \int_0^t u^{\bar{b}_2}_s ds$ and $t \mapsto \int_0^t (u^{\bar{b}_2}_s- w^{\bar{b}_2}_s)^2 (\bar{x}_s^{MFG,\bar{b}_2} )^2 ds$ are continuous over $[0,T]$ and differentiable over $(0,T)$. Then with the equation (\ref{eq:x_bar_MFG_explicit_new_notation}) we can apply the Mean Value theorem and deduce that there exists two fixed times $0< t_2 < t_1 < T$ such that:
	\begin{IEEEeqnarray}{rCl}
		\Delta SC &=& \frac{1}{2} B^{\bar{b}_2} \int_0^T (u^{\bar{b}_2}_t- w^{\bar{b}_2}_t)^2 (\bar{x}_t^{MFG, \bar{b}_2} )^2 dt  \nonumber \\
		&=& \frac{1}{2} B^{\bar{b}_2}  (u_{t_1}^{\bar{b}_2}- w_{t_1}^{\bar{b}_2} )^2 (\bar{x}_{t_1}^{MFG,\bar{b}_2} )^2  \cdot T \nonumber \\
		&=& \frac{T  B^{\bar{b}_2} }{2} \cdot (u_{t_1}^{\bar{b}_2}- w_{t_1}^{\bar{b}_2} )^2 \cdot \mathbb{E}[\xi]^2 \exp \left( 2 (b_1 + \bar{b}_1) t_1 - 2\int_0^{t_1} B^{\bar{b}_2} u_s^{\bar{b}_2} ds \right) \nonumber \\
		&=&  \kappa_1 (u_{t_1}^{\bar{b}_2}- w_{t_1}^{\bar{b}_2} )^2\cdot \left[ (b_2 + \bar{b}_2) \exp\left( - B^{\bar{b}_2} u_{t_2}^{\bar{b}_2} \cdot t_1 \right)  \right]^2 \nonumber \\
		&= & \kappa_1 (u_{t_1}^{\bar{b}_2} - w_{t_1}^{\bar{b}_2} )^2 \cdot \left((b_2 + \bar{b}_2) e^{- \kappa (b_2 + \bar{b}_2)^{\frac{1}{2}} h(\bar{b}_2)} \right)^2,
		\label{eq:delta_SC_b_2_bar_infty}
	\end{IEEEeqnarray}
	where  $\kappa = \frac{t_1}{r + \bar{r}(1-\bar{s})^2} >0 $ and $\kappa_1 =\frac{T \mathbb{E}[\xi]^2 e^{2 (b_1 + \bar{b}_1) t_1}}{2 (r + \bar{r}(1-\bar{s})^2)} >0$ are two positive constants independent of $\bar{b}_2$, and $ \displaystyle h(\bar{b}_2) = (b_2 + \bar{b}_2)^{\frac{3}{2}} u_{t_2}^{\bar{b}_2}$. 
