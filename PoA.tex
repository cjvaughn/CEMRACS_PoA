...
The following lemma will be used in Proposition \ref{prop:b2_to_infty}.
\begin{lemma}
...
\begin{proof}
	...
	Thus,
	$$1=b_2\eta^{b_2}_{t^*_{b_2}}-c_\eta=\frac{\frac{1}{b_2} \left[ToDO \right]}{\frac{1}{b_2^2} \left[ToDo \right]}-c_\eta=\frac{ToDo}{ToDo}.$$
	After rearranging terms:
	\begin{equation}
	\begin{split}
	\frac{D}{r+\bar{r}}-&\left[\frac{\delta^{-,b_2}_\eta}{b_2^2}+\frac{C^\eta}{b_2}+\frac{\delta^{-,b_2}_\eta c_\eta}{b_2^2}+D^\eta \left(\frac{\delta^{+,b_2}_\eta}{b_2}+c_{\delta^{b_2 \to \infty}_\eta} \right)\right] \\
	&= \left[-\frac{\delta^{-,b_2}_\eta D^\eta}{b_2}+\frac{D^\eta}{r+\bar{r}}(1+c_\eta)+ToDo \right] e^{-\left(\delta^{+,b_2}_\eta -\delta^{-,b_2}_\eta\right)(T-t)} \\
	&\leq  \left[-\frac{\delta^{-,b_2}_\eta D^\eta}{b_2}+\frac{D^\eta}{r+\bar{r}}(1+c_\eta) \right] e^{-\left(\delta^{+,b_2}_\eta -\delta^{-,b_2}_\eta\right)(T-t)}.
	\end{split}
	\label{eq:lemma_1}
	\end{equation}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\end{equation*}
	we have $c_u \neq c_w$. In this situation, we will show that the Price of Anarchy tends to a number strictly greater than $1$ as $b_2$ tends to infinity. To do so, we will show that in this asymptotic regime, $\Delta SC^{b_2}$ is lower bounded by a strictly positive constant and $SC^{MKV,b_2}$ is upper bounded by another constant, in which both of them are independent of $b_2$.\\
	
	We assume in the following that $b_2 \geq b_2^{*,basic} := \max \{b_2^{*,u,lower}, b_2^{*,\eta,lower}, b_2^{*,upper} \}$. We know directly that  $s \mapsto u_s^{b_2}$, $s \mapsto w_s^{b_2}$ and $s \mapsto \eta_s^{\eta}$ are increasing functions. Moreover, from equation (\ref{eq:ut_uniform_lower_bound_b2_infinity}) and (\ref{eq:eta_t_uniform_lower_bound_b2_infinity}), we have the uniform lower bounds $m_u$ and $m_\eta$ for $u_t^{b_2}$ and $\eta_t^{b_2}$.\\
	
	Step 1: We provide a lower bound for $\Delta SC^{b_2}$ with the similar techniques used for the term $I_1$, see inequality (\ref{eq:I1_ineq_b2_infinity}).   For every $0 \leq t <T$, we have $h_u(b_2, t) \xrightarrow[b_2 \to \infty]{} c_u$ and $h_w(b_2, t) \xrightarrow[b_2 \to \infty]{} c_w$. Let $\zeta_3 = \ln(2) \frac{r + \bar{r}(1-\bar{s})^2}{T (c_u + \vert c_u - c_w \vert / 4)} - \bar{b}_2$. Thus, there exists $b_2^{*,nominator} \geq \max \{ b_2^{*,basic}, \zeta_3 \}$ such that for all $b_2 \geq b_2^{*,nominator}$ and all $s \in [0, T/2]$ we have:
	\begin{equation*}
	\begin{array}{lclclclcl}
	...
	\end{equation*}
	which implies that
	$$ \left\vert h_u(b_2, s) - h_w(b_2,s)\right\vert \geq \frac{1}{2} \left\vert c_u - c_w \right\vert, \qquad \int_0^t h_u(b_2, s) ds \leq \left( c_u + \frac{1}{4} \left\vert c_u - c_w \right\vert \right) \cdot t.
	$$
	...
	\begin{IEEEeqnarray}{rCl}
		...
		&\geq & \frac{\left\vert c_u - c_w \right\vert^2 (\mathbb{E}(\xi))^2}{32 (c_u +  \vert c_u - c_w \vert / 4)}	
	\label{eq:nominator_ineq_b2_infinity}	
	\end{IEEEeqnarray}
	where the last inequality is justified by the choice of $\zeta_3$ so that
	$  1 - e^{- \frac{T (c_u + \frac{1}{4} \vert c_u - c_w \vert )}{r+\bar{r}(1-\bar{s})^2}\cdot (b_2 + \bar{b}_2)}  \geq \frac{1}{2}$. \\
	
	
	Step 2: We would like to provide an upper bound for $b_2\cdot SC^{MKV,b_2}$. From equation (\ref{eq:v_t_new_notation}), we have 
	\begin{equation}
	\begin{array}{rl}
		& \displaystyle b_2 \cdot SC^{MKV,b_2}   \\
		= & \displaystyle \underbrace{ \left[ \frac{q + \bar{q}}{2} \int_0^T b_2 v_t dt + \frac{q_T + \bar{q}_T}{2} b_2 v_T + \frac{b_2}{b_2+ \bar{b}_2} h_w(b_2, 0) (\mathbb{E}(\xi) )^2 \right] }_\text{\large{:= $J_1$}}  + \underbrace{ \frac{1}{2} \int_{0}^T  B^{\eta,b_2} (\eta_t^{b_2})^2 \cdot b_2 v_t dt }_\text{\large{:= $J_2$}}
	\end{array}		
	\end{equation}
