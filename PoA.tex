...
%	$$
	...
	\begin{IEEEeqnarray}{rCl}
		...
		&\geq& \frac{(D^{u,b_2 \to \infty} - \epsilon_1 ) \cdot (c_{\delta_u^{b_2 \to \infty}} - \epsilon_1) }{ \epsilon_1 \cdot (2 c_{\delta_u^{b_2 \to \infty}} + 2 \epsilon_1) + (D^{u,b_2 \to \infty} + \epsilon_1) / (r+ \bar{r}(1-\bar{s})^2) } =: m_u(\epsilon_1)>0
	...
	% check I_2
	Next, consider $I_2$ with $t^*(\epsilon)$ as in equation (\ref{eq:t^*_b2_infty}). 
	Since the functions $s \mapsto u_s^{\bar{b}_2}$, $s \mapsto w_s^{\bar{b}_2}$ and $s \mapsto \bar{x}_s^{MFG,\bar{b}_2}$ are continuous over $[t^*,T]$, then the functions $\displaystyle t \mapsto \int_{t^*}^T u^{\bar{b}_2}_s ds$ and $\displaystyle t \mapsto \int_{t^*}^T (u^{\bar{b}_2}_s- w^{\bar{b}_2}_s)^2 (\bar{x}_s^{MFG,\bar{b}_2} )^2 ds$ are continuous over $[t^*,T]$ and differentiable over $(t^*,T)$. We can apply the mean value theorem and deduce that for a fixed $b_2$, there exists a time point $t_1$ with $t^* < t_1 < T$ such that:
	\begin{equation}
		I_2 = (T-t^*) \cdot \frac{(u_{t_1}^{b_2} - w_{t_1}^{b_2})^2 e^{2(b_1 + \bar{b}_1)t_1} }{r + \bar{r}(1-\bar{s})^2}  \cdot  (b_2 + \bar{b}_2)^3 e^{ -\frac{2 (b_2 + \bar{b}_2)}{r + \bar{r}(1-\bar{s})^2} \int_0^{t_1} h_u(b_2,s) ds }.
	...
	and 
	\begin{equation*}
		\int_0^{t_1} h_u(b_2,s) ds \geq \int_0^{t^*} h_u(b_2,s) ds \geq t^* m_u(\epsilon_1).
	\end{equation*}
	Combining this with (\ref{eq:I2_b2_infity_Mean_Value}) gives:
	\begin{equation*}
		I_2 \leq  \frac{4 T M^2 e^{2(b_1 + \bar{b}_1) T} }{r + \bar{r}(1-\bar{s})^2} (b_2 + \bar{b}_2)^3 e^{- \frac{2 t^*  m_u(\epsilon_1)}{r + \bar{r}(1-\bar{s})^2} (b_2 + \bar{b}_2) } = \kappa_3 (b_2 + \bar{b}_2)^3 e^{- \kappa_4 (b_2 + \bar{b}_2)} \xrightarrow[b_2 \to \infty]{} 0,
	\end{equation*}
	where $\kappa_3 = \frac{4 TM^2 e^{2(b_1 + \bar{b}_1) T} }{r + \bar{r}(1-\bar{s})^2} $ and $\kappa_4 := \kappa_4(\epsilon, \epsilon_1) =  \frac{2 t^*  m_u(\epsilon_1)}{r + \bar{r}(1-\bar{s})^2}$ are two positive constants independent of $b_2$. 	
	...
	for any $\epsilon>0$, we can choose $t^*$ as in equation (\ref{eq:t^*_b2_infty}) so that for all $b_2 \geq b_2^*(\epsilon) := \max \{ b_2^{*,I_1}, b_2^{*,I_2}(\epsilon) \}$, the inequalities (\ref{eq:I1_b2_infty}) and (\ref{eq:I2_b2_infty}) yield:
	\begin{equation*}
