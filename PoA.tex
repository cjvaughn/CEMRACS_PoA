...
\begin{proof}
	...
	\begin{IEEEeqnarray*}{rl}
		...
		&\geq(-A^u + \vert A_u \vert) e^{-2T \sqrt{(A^u)^2 + c_1^2}}\\
...
\begin{proof}
	...
	% case when b_2 tends towards infinity
	...
	&\xrightarrow[b_2 \to \infty]{}& \displaystyle \pm \sqrt{\frac{q + \bar{q}(1-s)}{r + \bar{r}(1-\bar{s})} } =: \pm \delta^{b_2 \to \infty}_u
	\end{IEEEeqnarray*}
	Thus we have $\delta^{+,b_2}_u - \delta^{-,b_2}_u \xrightarrow[b_2 \to \infty]{} +\infty$ and for any $t \in [0,T)$,
	$$ e^{- (\delta^{+,b_2}_u - \delta^{-,b_2}_u)(T-t)} \xrightarrow[b_2 \to \infty]{} 0.$$
	
	Since $u^{b_2}$, $w^{b_2}$ and $\bar{x}^{b_2,MFG}$ are continuous functions on $[0,T]$, we have that the functions $t \mapsto \int_0^t u_s^{b_2} ds$ and $t \mapsto \int_0^t (u_s^{b_2}- w_s^{b_2})^2 (\bar{x}_s^{b_2, MFG})^2 ds$ are continuous over $[0,T]$ and differentiable over $(0,T)$. Then with equation (\ref{eq:x_bar_MFG_explicit_new_notation}) we can apply the Mean Value Theorem and deduce that there exists two fixed times $0 < t_2 < t_1 < T$ such that:
		\begin{IEEEeqnarray}{rCl}
		\Delta SC &=& \frac{1}{2} B^{b_2} \int_0^T (u^{b_2}_t- w^{b_2}_t)^2 (\bar{x}_t^{b_2,MFG} )^2 dt  \nonumber \\
		&=& \frac{1}{2} B^{b_2}  (u_{t_1}^{b_2}- w_{t_1}^{b_2} )^2 (\bar{x}_{t_1}^{\bar{b}_2,MFG} )^2  \cdot T \nonumber \\
		&=& \frac{T  B^{b_2} }{2} \cdot (u_{t_1}^{b_2}- w_{t_1}^{b_2} )^2 \cdot \mathbb{E}[\xi]^2 e^{ 2 (b_1 + \bar{b}_1) t_1 - 2\int_0^{t_1} B^{b_2} u_s^{\bar{b}_2} ds } \nonumber \\
		&=&  \kappa_1 (u_{t_1}^{b_2}- w_{t_1}^{b_2} )^2\cdot \left[ (b_2 + \bar{b}_2) e^{- B^{b_2} u_{t_2}^{b_2} \cdot t_1}  \right]^2 \nonumber \\
		&= & \kappa_1 (u_{t_1}^{b_2} - w_{t_1}^{b_2} )^2 \cdot \left((b_2 + \bar{b}_2) e^{- \kappa (b_2 + \bar{b}_2) f(b_2,t_2)} \right)^2,
		\label{eq:delta_SC_b_2_infty}
	\end{IEEEeqnarray}
	where  $\kappa = \frac{t_1}{r + \bar{r}(1-\bar{s})^2} >0 $ and $\kappa_1 =\frac{T \mathbb{E}[\xi]^2 e^{2 (b_1 + \bar{b}_1) t_1}}{2 (r + \bar{r}(1-\bar{s})^2)} >0$ are two positive constants independent of $b_2$, and $f(b_2,t) = (b_2 + \bar{b}_2) u_{t}^{b_2}$. 
...
\begin{proof}
	...
	\begin{IEEEeqnarray}{rCl}
		\Delta SC &=& \frac{1}{2} B^{\bar{b}_2} \int_0^T (u_t- w_t)^2 (\bar{x}_t^{\bar{b}_2,MFG} )^2 dt  \nonumber \\
		&=& \frac{1}{2} B^{\bar{b}_2}  (u_{t_1}^{\bar{b}_2}- w_{t_1}^{\bar{b}_2} )^2 (\bar{x}_{t_1}^{\bar{b}_2,MFG} )^2  \cdot T \nonumber \\
		...
		&= & \kappa_1 (u_{t_1}^{\bar{b}_2} - w_{t_1}^{\bar{b}_2} )^2 \cdot \left((b_2 + \bar{b}_2) e^{- \kappa (b_2 + \bar{b}_2)^{\frac{1}{2}} f(\bar{b}_2,t_2)} \right)^2,
	...
	\end{array}
	\end{equation*}
	Let $\epsilon>0$. Then there exists a $\bar{b}_2^{**}>\bar{b}_2^{*}$, such that for all $\bar{b}_2>\bar{b}_2^{**}$, we have:
	\begin{equation}
	|(b_2 + \bar{b}_2) e^{- \kappa (b_2 + \bar{b}_2)^{\frac{1}{2}} f(\bar{b}_2,t_2) }|<\epsilon.
	\label{eq:limit_p_f(p)}
	\end{equation}
 	
 	
 	Now, in order to show that $\Delta SC$ tends to $0$, we need to bound the terms $u_{t_1}^{\bar{b}_2}$ and $w_{t_1}^{\bar{b}_2}$ for large enough $\bar{b}_2$. Given $\epsilon>0$, there exists a $\bar{b}_2^*:=\bar{b}_2^*(\epsilon,t_1) > 1$ such that for any $\bar{b}_2 \geq \bar{b}_2^*$ we have $ (b_2 + \bar{b}_2)^{\frac{3}{2}} \vert u_{t_1}^{\bar{b}_2} \vert \leq c_3 + \epsilon$. Thus, for any $\bar{b}_2 \geq \bar{b}_2^*$ we deduce:
	...
	Hence, together with equation (\ref{eq:delta_SC_b_2_bar_infty}) and  (\ref{eq:limit_p_f(p)}), we obtain that for any $\epsilon > 0$, there exists a $\bar{b}_2^*$ large enough so that for all $\bar{b}_2 \geq \bar{b}_2^*$ we have
	$$ \Delta SC \leq \kappa_1 (c_3 + c_4 + 2 \epsilon)^2 \epsilon^2.$$
	Thus, by taking $\epsilon \to 0$, we have:
	$$\lim_{\bar{b}_2 \to \infty} \Delta SC = 0.$$
	

% error in the proof of proposition 5
\begin{comment}	
	...
	%\end{equation*}
	%From equation (\ref{eq:x_bar_MFG_explicit_new_notation}), we deduce:
