...
\begin{equation*}
\begin{split}
= & 2 \int_0^T (\bar{\eta}^{MFG}_t)^2 (\bar{x}^{MFG}_t)^2dt  - \frac{1}{c^{MFG}(b_2 + \bar{b}_2)} \Bigg[ \bar{\eta}^{MFG}_T (\bar{x}^{MFG}_T)^2 - \bar{\eta}^{MFG}_0 (\bar{x}^{MFG}_0)^2 \\
...
\begin{equation*}
\begin{split}
\int_0^T (\bar{\eta}^{MFG}_t)^2 (\bar{x}^{MFG}_t)^2 dt= \frac{1}{c^{MFG}(b_2 + \bar{b}_2)} &\Bigg[ \bar{\eta}^{MFG}_T (\bar{x}^{MFG}_T)^2 - \bar{\eta}^{MFG}_0 (\bar{x}^{MFG}_0)^2 \\
&+ \int_0^T  (-\bar{b}_1 \bar{\eta}^{MFG}_t+(q+\bar{q}(1-s))) (\bar{x}^{MFG}_t)^2 dt \Bigg].
\end{split}
\end{equation*}
If we denote:
...
& + \frac{1}{2} \lambda \left( \bar{\eta}^{MFG}_0 \bar{x}_0^2 - (q_T+\bar{q}_T(1-s_T)) (\bar{x}_T^{MFG})^2 \right) + \frac{1}{2} (q_T+\bar{q}_T(1-s_T)^2)(\bar{x}_T^{MFG})^2.
\end{split}
\label{eq:SC_MFG_2}
\end{equation}
...
\begin{equation*}
\begin{split}
\int_0^T (\bar{\eta}^{MKV}_t)^2 (\bar{x}^{MKV}_t)^2 dt=\frac{1}{c^{MKV}(b_2 + \bar{b}_2)} &\Bigg[ \bar{\eta}^{MKV}_T (\bar{x}^{MKV}_T)^2 - \bar{\eta}^{MKV}_0 (\bar{x}^{MKV}_0)^2 \\
...
\begin{equation}
SC^{MKV} = h_{var} + \frac{1}{2} \bar{\eta}^{MKV}_0 \bar{x}_0^2.
\label{eq:SC_MKV_2}
\end{equation}
...
\begin{proof}
	...
	\right.
	\end{equation*} 
	Since $q_T + \bar{q}_T(1-s_T)^2 = \bar{\eta}_T^{MKV}$ and $\lambda \bar{\eta}_0^{MFG} - \bar{\eta}_0^{MKV} = \Delta\bar{\eta}_0$, we deduce from equations (\ref{eq:SC_MFG_2}) and (\ref{eq:SC_MKV_2}) that:
	\begin{equation*}
		\begin{split}
			& SC^{MFG} - SC^{MKV}\\
...
\end{corollary}
\begin{proof}
    By the explicit equation for $\bar{x}_t$ (see Appendix \ref{appendix_solving_fbsdes}), $\bar{x}_0=0$ implies $\bar{x}_t=0,\ \forall t \in [0,T]$. Therefore by Proposition \ref{prop:diff_SC}, $\Delta SC=0$. From the explicit formula for the variance, $h_{var}>0$ and therefore by equation (\ref{eq:SC_MKV_2}), $SC^{MKV}>0$. Therefore, $PoA=1$.
\end{proof}
...
\begin{proof}
	...
	\end{split}
	\end{equation*}
	and using the fact that $w^{0,b_2}_t\not \equiv 0$  and using equation (\ref{eq:SC_MKV_2}), $ \lim_{b_2\to 0} SC^{MKV} >0 $.
...
\begin{proof}
	...
	\begin{equation*}
    \begin{split}
	    \lim_{\bar{b}_2 \to 0}SC^{MKV} &= h_{var} + w^{0,\bar{b}_2}_0 \bar{x}_0^2, \\
