...
\begin{proof}
	...
	Moreover, we have $\lim_{b_2 \to 0}\delta^{+,b_2}_u =-2A^u >0 $ and $\lim_{b_2 \to 0}\delta^{-,b_2}_u = 0$, so that $\lim_{b_2 \to 0}(\delta^{+}_u - \delta^{-}_u) = -2 A^u > 0$. Thus, from equation (\ref{eq:riccati_ut_sol}) we deduce that for every fixed time $t \in [0,T]$, $\lim_{b_2 \to 0}u^{b_2}_t = 0.$
	...
	and thus, $ 0 < \lim_{b_2 \to 0} SC^{MKV,b_2} < \infty.$
	Therefore, combined with equation (\ref{eq:PoA_LQ_new_notation}), we conclude that: $$\lim_{b_2 \to 0} PoA^{b_2} =: PoA^{b_2 \to 0} > 1.$$
\end{proof}
...
\begin{remark}
	...
	$$D^{u,\bar{b}_2 \to \infty} = D^w=:D \quad \text{and} \quad B^{\bar{b}_2 \to 0} D^2 + 2 A^{u}D^{u,\bar{b}_2 \to 0} - C^{u,\bar{b}_2 \to 0} = B^{\bar{b}_2 \to 0} D^2 + 2 A^{w} D^w - C^w  = 0.$$
	In other words, the following three equations imply $\lim_{\bar{b}_2 \to 0} PoA^{\bar{b}_2 } =1$:
...
\begin{proof}
	...
	$\lim_{\bar{b}_1 \to \infty}\frac{u^{\bar{b}_1}_t}{\bar{b}_1}=\frac{1}{B}$. By the same arguments, we have for $t \in [0,T)$: $\lim_{\bar{b}_1 \to \infty}\frac{w^{\bar{b}_1}_t}{\bar{b}_1}=\frac{2}{B}$.
