...
\begin{proof}
	...
	In order to show that $\lim_{r \to \infty} PoA^r = 1$, it suffices to show that $\lim_{r \to \infty}\Delta SC^{r} = 0$ while $\lim_{r \to \infty} SC^{r, MKV} >0$.
	...
	$$
	The variance function $v_t^{r}$ is also uniformly bounded over $t \in [0,T]$ for large $r$.	It can be shown that $w^{r \to \infty }_0\geq0$ and $v^{r \to \infty}_t >0$ for $t>0$. Hence,
	\begin{equation*}
