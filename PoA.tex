...
\begin{proposition}
	Assuming (\ref{eq:assumptions}), the initial condition $\xi$ satisfies $\mathbb{E}(\xi)\neq 0$, and assuming $\bar{b}_1> 0$, $b_2> 0$, $q_T+\bar{q}_T(1-s_T)>0$, and $q_T+\bar{q}_T(1-s_T)^2>0$ then 
	\begin{equation*}
...
\begin{proof}
	...
	\end{split}
	\end{equation*}
	%Similar to the proof of Proposition \ref{prop:r_bar_r}, and using the assumptions $b_2>0$ and $q_T+\bar{q}_T(1-s_T)>0$, it can be shown that there exists a $c>0$ and $\bar{b}_2^*$ such that for $\bar{b}_2>\bar{b}^*_2$ and $\forall t \in [0,T]$:
	%\begin{equation*}
	%\begin{split}
	%|(b_2 + \bar{b}_2)^{\frac{3}{2}}\cdot u^{\bar{b}_2}_t| <& c, \\
	%|(b_2 + \bar{b}_2) w^{\bar{b}_2}_t| <& c, \\
	%|\bar{x}_t^{\bar{b}_2,MFG}|<&c.
	%\end{split}
	%\end{equation*}
	
	Next, our goal is to derive bounds for the functions $\frac{(b_2 + \bar{b}_2)^{\frac{3}{2}}\cdot u^{\bar{b}_2}_t}{\sqrt{b_2+\bar{b}_2}}$, $(b_2 + \bar{b}_2) w^{\bar{b}_2}_t$, and $\bar{x}_t^{\bar{b}_2,MFG}$. Let $0<\epsilon<\delta^{\bar{b}_2 \to \infty}$. We can check that there exists a $\bar{b}_2^*$ such that $ C^{u,\bar{b}_2}\sqrt{b_2 + \bar{b}_2} <\epsilon$, and $\delta^{\bar{b}_2 \to \infty}-\epsilon<\left| \frac{\delta^{\pm,\bar{b}_2}_u}{\sqrt{b_2 + \bar{b}_2}}\right|< \delta^{\bar{b}_2 \to \infty} +\epsilon$ for $\bar{b}_2>\bar{b}_2^*$. Then we have for $t\in[0,T)$:
		\begin{equation*}
		\begin{split}
		...
		\end{split} 
		\end{equation*}
	ToDo: other 2 function bounds
	
	Hence, by the bounded convergence theorem, we deduce:
