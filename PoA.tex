...
\begin{split}
    ...
    (b^{MFG}-b^{MKV})(b_2+\bar{b}_2)\bar{\eta}_t^{MKV}\bar{x}_t^{MKV}&=0,\ \forall t \in [0,T] \\
...
\begin{proof}
    Comparing the FBSDE systems (\ref{eq:FBSDE_EMFG}) and (\ref{eq:FBSDE_EMKV}), and using the fact that $c^{MFG}-c^{MKV}=b^{MFG}-b^{MKV}$, the result is clear.
\end{proof}

\begin{corollary}
Assuming (\ref{eq:assumptions}), if furthermore, $s\bar{q}(s-1)=0$, $\bar{b}_1=0$, $\frac{b_2(r+\bar{r}(1-\bar{s})^2)}{(b_2+\bar{b}_2)(r+\bar{r}(1-\bar{s}))}=1$, and $s_T\bar{q}_T(s_T-1)=0$,
then $PoA=1$.
\label{corollary_1}
\end{corollary}
