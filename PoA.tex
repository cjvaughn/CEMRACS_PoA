...
\begin{proof}
	...
	A^{u}, A^{w}, C^w, D^w, A^{\eta}, C^{\eta}, D^{\eta} \text{ are independent of } b_2
	\end{array}
	\end{equation*}
	Moreover
	\begin{IEEEeqnarray*}{rCl}
		\frac{\delta_u^{\pm,b_2}}{b_2 + \bar{b}_2} &=& \frac{A^u}{b_2 + \bar{b}_2} \pm \sqrt{\frac{C^{u,b_2}}{r + \bar{r}(1-\bar{s})^2} + \frac{(A^u)^2}{(b_2+\bar{b}_2)^2} }\\
		&\xrightarrow[b_2 \to \infty]{}& \displaystyle \pm \sqrt{\frac{q + \bar{q}(1-s)}{r + \bar{r}(1-\bar{s})} } =: \pm c_{\delta_u^{b_2 \to \infty}}
	\end{IEEEeqnarray*}
	Thus we have $\delta^{+,b_2}_u - \delta^{-,b_2}_u \xrightarrow[b_2 \to \infty]{} +\infty$ and for any $t \in [0,T)$,
	...
	\begin{IEEEeqnarray*}{rCl}
		...
		&&\\
		&\xrightarrow[b_2 \to \infty]{}& \quad (r + \bar{r}(1-\bar{s})^2) \cdot c_{\delta_u^{b_2 \to \infty}}  =: c_3
	\end{IEEEeqnarray*}
	Idem, we can have for any $t \in [0,T)$,
	$$ (b_2 + \bar{b}_2) w_t^{b_2} \xrightarrow[b_2 \to \infty]{}  (r + \bar{r}(1-\bar{s})^2) \cdot c_{\delta_w^{b_2 \to \infty}} =: c_4,\qquad \text{with } \quad c_{\delta_w^{b_2 \to \infty}} = \sqrt{ \frac{q + \bar{q}(1-s)^2}{r + \bar{r}(1-\bar{s})^2} };$$
...
\begin{proof}
	...
	From equation (\ref{eq:x_bar_MFG_explicit_new_notation}), the assumption on $\xi$, and by the bounded convergence theorem, we have for every $t \in [0,T]$:
   	$$\lim_{\bar{b}_2 \to 0} \bar{x}_t^{MFG,\bar{b}_2} = \mathbb{E}(\xi) e^{\int_0^t (b_1 + \bar{b}_1 - B^{\bar{b}_2 \to 0}u^{\bar{b}_2 \to 0}_s) ds} =: \bar{x}^{MFG,\bar{b}_2 \to 0}_t>0.$$
...
%	$$\lim_{\bar{b}_2 \to \infty} \delta^{\pm,\bar{b}_2}_u=\pm \infty \qquad \text{and } \qquad \lim_{\bar{b}_2 \to \infty} \frac{\delta^{\pm,\bar{b}_2}_u}{b_2 + \bar{b}_2}=0.$$
	...
	\begin{equation}
	 \sup_{\bar{b}_2 \geq \bar{b}_2^{*,1}} \sup_{t \in [0,T]} \max \left\{ \vert u_t^{\bar{b}_2}\vert, \vert w_t^{\bar{b}_2} \vert \right\} \leq M(\epsilon_1).
	...
	and then we bound separately the following terms in terms of $\epsilon$:
	...
	\begin{IEEEeqnarray}{rCl}
		I_1 &=& \frac{1}{2} B^{\bar{b}_2} \int_0^{\frac{T}{2}}  (u^{\bar{b}_2}_s- w^{\bar{b}_2}_s)^2 (\bar{x}_s^{MFG, \bar{b}_2} )^2 ds  \nonumber \\
	...
	% the term I_2
	Now, we look the term $I_2$. Since $u_t^{\bar{b}_2}$, $w_t^{\bar{b}_2}$ and $\bar{x}_t^{MFG,\bar{b}_2}$ are continuous over $[\frac{T}{2},T]$, then the functions $t \mapsto \int_0^t u^{\bar{b}_2}_s ds$ and $t \mapsto \int_0^t (u^{\bar{b}_2}_s- w^{\bar{b}_2}_s)^2 (\bar{x}_s^{MFG,\bar{b}_2} )^2 ds$ are continuous over $[\frac{T}{2},T]$ and differentiable over $(\frac{T}{2},T)$. Then with the equation (\ref{eq:x_bar_MFG_explicit_new_notation}) we can apply the Mean Value theorem and deduce that there exists two fixed times $t_1$ and $t_2$ depending on $\bar{b}_2$ which satisfy $\frac{T}{2} < t_1 < T$ and $0 < t_2 < t_1 <T$ such that:
	\begin{IEEEeqnarray}{rCl}
		...
		&\leq & \kappa_3 (u_{t_1}^{\bar{b}_2} - w_{t_1}^{\bar{b}_2} )^2 \cdot (b_2 + \bar{b}_2)^2 \exp \left( - \kappa_4 (b_2 + \bar{b}_2)^{\frac{1}{2}} h(\bar{b}_2,t_2) \right),
	...
	Hence, let $\bar{b}_2^{*,3} = \max \{\bar{b}_2^{*,upper}, \bar{b}_2^{*,2}, \bar{b}_2^{*,lower} \}$ and together with inequality (\ref{eq:u_t_w_t_uniform_bound_b2_bar_infty}) and (\ref{eq:I_2_b2_bar_inequality}), we obtain that for all $\bar{b}_2 \geq \bar{b}_2^{*,3}$:
	\begin{IEEEeqnarray*}{rCl}
	...
	Hence, from inequality (\ref{eq:I_1_b2_bar_infinity}) and (\ref{eq:I_2_b2_bar_infinity}), for all $\bar{b}_2 \geq \bar{b}_2^* := \max \{ \bar{b}_2^{*,I_1}, \bar{b}_2^{*,I_2}\}$ we have:
	\begin{equation*}
		\Delta SC = I_1 + I_2 \leq 2\epsilon.
	\end{equation*} 
	Therefore, by taking $\epsilon \to 0$, we obtain:
	\begin{equation*}
		\lim_{\bar{b}_2 \to \infty} \Delta SC = 0.
	\end{equation*}
	
		
% comment out: error in using Mean Value theorem	
\begin{comment}
	-----------------
	
	Our next step is to show that $(b_2 + \bar{b}_2) e^{- \kappa (b_2 + \bar{b}_2)^{\frac{1}{2}} h(\bar{b}_2) } $ tends towards $0$ when $\bar{b}_2 \to \infty$. We previously showed that $\lim_{\bar{b}_2 \to \infty}h(\bar{b}_2)= c_3 >0$. Let $\epsilon_1>0$ such that $c_3 - 2 \epsilon_1 >0$. Then there exists $\bar{b}_2^{*,1}>0$ so that for all $\bar{b}_2 > \bar{b}_2^{*,1}$ we have $\left\vert h(\bar{b}_2) - c_3 \right\vert < \epsilon_1$. Then $h(\bar{b}_2) - c_3 + 2\epsilon_1 > \epsilon_1$ and 
	\begin{equation*}
	\begin{array}{rl}
	(b_2 + \bar{b}_2) e^{- \kappa (b_2 + \bar{b}_2)^{1/2} h(\bar{b}_2) }  & 	\displaystyle  = e^{- \kappa (b_2 + \bar{b}_2)^{1/2} (h(\bar{b}_2) - c_3 + 2\epsilon_1 )}\cdot e^{- \kappa  (c_3 - 2 \epsilon_1) (b_2 + \bar{b}_2)^{1/2} +  \ln (b_2 + \bar{b}_2) } \\
		&	\displaystyle  \leq  e^{- \kappa (b_2+\bar{b}_2^{*,1})^{1/2} \epsilon_1 } \cdot \exp \left( - \kappa (c_3 - 2 \epsilon_1 ) (b_2 + \bar{b}_2)^{\frac{1}{2}} + \ln (b_2 + \bar{b}_2)\right) \\
		%& 	\displaystyle  = c_5 e^{- \kappa (c_3 - 2 \epsilon_1) (b_2 + \bar{b}_2)^{1/2} + \ln (b_2 + \bar{b}_2)} \\
		%& \xrightarrow[\bar{b}_2 \to \infty]{}0.
	\end{array}
	\end{equation*}
	The first term on the right hand side of the above inequality is a constant depending only on $\epsilon_1$, and the second term tends towards $0$ when $b_2 \to \infty$. Thus,  for any $\epsilon>0$, there exists a $\bar{b}_2^{*,2} \geq \bar{b}_2^{*,1}$, such that for all $\bar{b}_2>\bar{b}_2^{*,2}$ we have:
	\begin{equation}
	|(b_2 + \bar{b}_2) e^{- \kappa (b_2 + \bar{b}_2)^{\frac{1}{2}} h(\bar{b}_2) }|<\epsilon.
	\label{eq:limit_p_f(p)}
	\end{equation}
 	
 	
 	Now, in order to show that $\Delta SC$ tends to $0$, we need to bound the terms $u_{t_1}^{\bar{b}_2}$ and $w_{t_1}^{\bar{b}_2}$ for large enough $\bar{b}_2$. Given $\epsilon>0$, there exists a $\bar{b}_2^{*,3} \geq 1$ such that for any $\bar{b}_2 > \bar{b}_2^{*,3}$ we have $ (b_2 + \bar{b}_2)^{\frac{3}{2}} \left\vert u_{t_1}^{\bar{b}_2} \right\vert \leq c_3 + \epsilon$. Thus, for any $\bar{b}_2 > \bar{b}_2^{*,3} \geq 1$ we deduce:
 	$$ \left\vert u_{t_1}^{\bar{b}_2} \right\vert \leq \frac{c_3 + \epsilon}{(b_2 + \bar{b}_2)^{\frac{3}{2}} } \leq \frac{c_3+ \epsilon}{(b_2 + \bar{b}^{*,3}_2)^{\frac{3}{2}} } \leq c_3 + \epsilon.
 	$$
	...
	\end{split}
	\end{equation*}
	where we have used the fact that $\lim_{\bar{b}_2 \to \infty}\frac{\delta^{\pm,\bar{b}_2}_w}{b_2 + \bar{b}_2}=\sqrt{\frac{C^w}{r+\bar{r}(1-\bar{s})^2}}$. 
	
	
	Idem we can show that for any $t_1 \in [0,T)$:
	\begin{equation*}
		(b_2 + \bar{b}_2) w^{\bar{b}_2}_{t_1} \xrightarrow[\bar{b}_2 \to \infty]{} (r+ \bar{r}(1-\bar{s})^2) c_{\delta_w^{\bar{b}_2 \to \infty}} =: c_4, \qquad \text{where } \quad c_{\delta_w^{\bar{b}_2 \to \infty}} = \sqrt{\frac{q+ \bar{q}(1-s)^2}{r + \bar{r}(1 - \bar{s})^2} }.
	\end{equation*}
	Thus, $\left\vert w_{t_1}^{\bar{b}_2} \right\vert $ can also be bounded by $c_4 + \epsilon$ for any $\bar{b}_2$ large enough.
	
	Hence, together with equation (\ref{eq:delta_SC_b_2_bar_infty}) and  (\ref{eq:limit_p_f(p)}), we obtain that for any $\epsilon > 0$, there exists a $\bar{b}_2^*$ large enough so that for all $\bar{b}_2 > \bar{b}_2^*$ we have
...
\begin{proof}
	...
	From equation (\ref{eq:x_bar_MFG_explicit_new_notation}), the assumption on $\xi$, and by the bounded convergence theorem, we have for every $t \in [0,T]$:
	\begin{equation*}
	\lim_{b_1\to 0}\bar{x}_t^{MFG,b_1} = \mathbb{E}(\xi) e^{\int_0^t(\bar{b}_1-B u^{b_1\to 0}_s)ds} =: \bar{x}_t^{MFG,b_1\to 0}>0.
	\end{equation*}
