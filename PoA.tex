...
\begin{proposition}
	Assuming (\ref{eq:assumptions}), $q_T+\bar{q}_T(1-s_T)>0$, $q_T+\bar{q}_T(1-s_T)^2>0$, and the initial condition $\xi$ satisfies $\mathbb{E}(\xi)\neq 0$, then:
...
%	$$
	...
	Next, we derive a uniform lower bound for $(b_2 + \bar{b}_2) u_t^{b_2}$ over $[0,T]$ and large $b_2$. Let $\epsilon_1 > 0$ so that $\epsilon_1 < \min \left\{ c_{\delta_u^{b_2 \to \infty}}, D^{u,b_2 \to \infty} \right\}$. Then there exists $b_2^{*,lower} := b_2^{*,lower}(\epsilon_1) 	>0$ such that for all $b_2 \geq b_2^{*,lower}$,
	$$ 
	\max \left\{  \left\vert \frac{\delta_u^{+,b_2}}{b_2 + \bar{b}_2}  - c_{\delta_u^{b_2 \to \infty}} \right\vert, \   \left\vert \frac{\delta_u^{-,b_2}}{b_2 + \bar{b}_2}  - \left(-c_{\delta_u^{b_2 \to \infty}}\right) \right\vert, \vert D^{u,b_2} - D^{u,b_2\to \infty} \vert,  \left\vert \frac{1}{b_2 + \bar{b}_2} \right\vert  \right\} \leq \epsilon_1,
	$$ 
	and thus,
	\begin{IEEEeqnarray}{rCl}
		(b_2 + \bar{b}_2) u_t^{b_2} &\geq&  (b_2 + \bar{b}_2) \frac{D^{u,b_2} \delta_u^{+, b_2} }{ (\delta_u^{+,b_2} - \delta_u^{-,b_2}) + B^{b_2}D^{u,b_2} } \nonumber \\
		&=& \frac{D^{u,b_2} \frac{\delta_u^{+, b_2}}{b_2+ \bar{b}_2} }{ \frac{1}{b_2 + \bar{b}_2} \cdot \frac{\delta_u^{+,b_2} - \delta_u^{-,b_2}}{b_2 + \bar{b}_2} + \frac{D^{u,b_2}}{r + \bar{r}(1-\bar{s})^2} } \nonumber \\
		&\geq& \frac{(D^{u,b_2 \to \infty} - \epsilon_1 ) \cdot (c_{\delta_u^{b_2 \to \infty}} - \epsilon_1) }{ \epsilon_1 \cdot (2 c_{\delta_u^{b_2 \to \infty}} + 2 \epsilon_1) + (D^{u,b_2 \to \infty} + \epsilon_1) / (r+ \bar{r}(1-\bar{s})^2) } =: m_u(\epsilon_1)
	...
	Also, from equation (\ref{eq:riccati_ut_sol}), for any fixed $b_2$, we can compute the derivative of $u_t^{b_2}$ with respect to time $t$:
	\begin{equation*}
		\frac{d u_t^{b_2}}{dt} = \frac{\left(B^{b_2} (D^{u,b_2})^2 + 2 A^u D^{u,b_2} - C^{u,b_2} \right) \cdot \left(\delta_u^{+,b_2} - \delta_u^{-,b_2}\right)^2 e^{- (\delta_u^{+,b_2} - \delta_u^{-,b_2})(T-t) } }{\left[B^{b_2} D^{u,b_2}(1-e^{- (\delta_u^{+,b_2} - \delta_u^{-,b_2})(T-t)})   + \delta_u^{+,b_2}e^{- (\delta_u^{+,b_2} - \delta_u^{-,b_2})(T-t)}- \delta_u^{-,b_2}  \right]^2 }.
	\end{equation*}
	Hence, the function $t \mapsto u_t^{b_2}$ is increasing if $B^{b_2} (D^{u,b_2})^2 + 2 A^u D^{u,b_2} - C^{u,b_2} > 0$. Since 
	...
	$$ \sup_{b_2 \geq b_2^{*,u,upper}} \sup_{0\leq t \leq T} \left\{ u_t^{b_2} \right\} = \sup_{b_2 \geq b_2^{*,u,upper}} \left\{ u_T^{b_2}\right\}  = \sup_{b_2 \geq b_2^{*,u,upper}} \{ D^{u,b_2}  \} \leq D^{u,b_2 \to \infty} + 1.$$ (ToDo: why 1 and not $\epsilon_1$?)
	Idem for $t \mapsto w_t^{b_2}$. Let $b_2^{*,upper} = \max \{ b_2^{*,u,upper}, b_2^{*,w,upper}\}$ and $M = \max \{  D^{u,b_2 \to \infty}+1, D^w\}$. Then for $b_2\geq b_2^{*,upper}$, we have the uniform upper bound:
	\begin{equation}
	\sup_{b_2 \geq b_2^{*,upper}} \sup_{0 \leq t \leq T} \max \left\{ u_t^{b_2} , w_t^{b_2}\right\} \leq M.
	\label{eq:ut_wt_upper_bound_b2_infty}
	\end{equation}
	(ToDo: why should we write the uniform upper bounds together? We can give them individually.)
...
%	$$\lim_{\bar{b}_2 \to \infty} \delta^{\pm,\bar{b}_2}_u=\pm \infty \qquad \text{and } \qquad \lim_{\bar{b}_2 \to \infty} \frac{\delta^{\pm,\bar{b}_2}_u}{b_2 + \bar{b}_2}=0.$$
	...
	\begin{IEEEeqnarray*}{rCl}
		\vert u_t^{\bar{b}_2} \vert &=& \frac{\displaystyle C^{u,\bar{b}_2} \left(1 - e^{-(\delta^{+,\bar{b}_2}_u - \delta^{-,\bar{b}_2}_u)(T-t)} \right) + D^{u,\bar{b}_2} \left(\delta_u^{+,\bar{b}_2} - \delta_u^{-,\bar{b}_2} e^{- (\delta^{+,\bar{b}_2}_u - \delta^{-,\bar{b}_2}_u)(T-t)} \right) }{\displaystyle B^{\bar{b}_2} D^{u,\bar{b}_2} \left( 1 - e^{-(\delta^{+,\bar{b}_2}_u - \delta^{-,\bar{b}_2}_u)(T-t)} \right)+ \delta_u^{+,\bar{b}_2} e^{- (\delta^{+,\bar{b}_2}_u - \delta^{-,\bar{b}_2}_u)(T-t)} - \delta_u^{-,\bar{b}_2} } \\
