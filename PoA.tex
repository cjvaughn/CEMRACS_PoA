...
\end{equation}
where $ h_{var}: = \frac{1}{2}\int_0^T (q+\bar{q}+(r+\bar{r})(a\eta_t)^2)v_tdt + \frac{1}{2}(q_T+\bar{q}_T)v_T  $.
...
\end{corollary}
\begin{proof}
    By the explicit equation for $\bar{x}_t$ (see Appendix \ref{appendix_solving_fbsdes}), $\bar{x}_0=0$ implies $\bar{x}_t=0,\ \forall t \in [0,T]$. Therefore by Proposition \ref{prop:diff_SC}, $\Delta SC=0$. From the explicit formula for the variance, $h_{var}>0$ and therefore by equation (\ref{eq:SC_MFG_MKV_2}), $SC^{MKV}>0$. Therefore, $PoA=1$.
\end{proof}
