...
\begin{proof}
	...
	If $A^u \neq 0$, let $u^{\infty,r}:[0,T] \to \mathbb{R}$ be the solution to the first-order differential equation:
	\begin{equation}
		\dot{u}^{\infty,r}_t - 2A^u u_t^{\infty,r} + C^{\infty,r}_u = 0, \qquad u_T^{\infty,r} = D^{\infty,r}_u,
	\end{equation}
	or if $A^u = 0$, let $u^{\infty,r}:[0,T] \to \mathbb{R}$ solve:
	\begin{equation}
		\dot{u}^{\infty,r}_t + C^{\infty,r}_u = 0, \qquad u_T^{\infty,r} = D^{\infty,r}_u.
	\end{equation}	
	Since assumption (\ref{eq:assumptions}) holds for arbitrarily large $r$, from the explicit formula of the solution to the scalar Riccati equation (see Appendix \ref{appendix_solving_fbsdes} equation (\ref{eq:riccati_ut_sol_app})), it is easy to show that the solution $u$ to equation (\ref{eq:riccati_ut}) converges pointwise for any $t\in[0,T]$ to the solution $u^{\infty,r}$, as $r \to \infty$. This limiting solution $u^{\infty,r}$ can be given by the following expressions: for every $t\in [0,T]$
	\begin{equation*}
	\begin{split}
	...
	It is clear that $u^{\infty,r}$ is bounded over $[0,T]$.	Similarly, for any fixed time $t\in [0,T]$, the solutions $w_t$ and $\eta_t$ of equations (\ref{eq:riccati_wt}) and (\ref{eq:riccati_etat}) converge for each time $t\in [0,T]$ towards the solutions $w^{\infty,r}_t$ and $\eta_t^{\infty,r}$ of some first-order differential equations. Namely, for every $t\in [0,T]$,
	$$w_t \xrightarrow[r \to \infty]{} w_t^{\infty,r},\qquad \eta_t \xrightarrow[r \to \infty]{} \eta_t^{\infty,r}$$
	and the functions $w^{\infty,r}$ and $\eta^{\infty,r}$ are bounded over $[0,T]$.
	
	Moreover, since $\bar{x}_t^{MFG} = \mathbb{E}(\xi) \exp(\int_0^t (b_1 + \bar{b}_1 - B\cdot u_s)ds)$ and $v_t = Var(\xi)\exp(2\int_0^t (b_1 - B^\eta \eta_s) ds) + \sigma^2 \int_0^t \exp(2 \int_s^t (b_1 - B^\eta \eta_u) du) ds$, by the bounded convergence theorem we have: 
	$$	\bar{x}_t^{MFG} \xrightarrow[r\to \infty]{} \mathbb{E}(\xi) e^{(b_1+ \bar{b}_1)t} =: \bar{x}_t^{\infty,r,MFG},\qquad
	  v_t \xrightarrow[r\to \infty]{} Var(\xi)e^{2b_1t} + \sigma^2 \int_0^t e^{2 b_1 (t-s) } ds =: v^{\infty,r}_t,
	$$
	The limits $\bar{x}_t^{\infty,r,MFG}$ and $v_t^{\infty,r}$ are bounded over $[0,T]$. By the bounded convergence theorem, we obtain:
	\begin{equation*}
	...
	\end{equation*}
	It can be shown that $w^{\infty,r}_0\geq0$ and $v^{\infty,r}_t >0$ for $t>0$. Hence,
	\begin{equation*}
		\lim_{r \to \infty} SC^{MKV} = \int_0^T (q+\bar{q}) v^{\infty,r}_t dt + (q_T + \bar{q}_T) v^{\infty,r}_T + w^{\infty,r}_0 (\mathbb{E}(\xi))^2 \  >0.
	\end{equation*}
	Therefore, from Proposition \ref{prop:diff_SC} and equation (\ref{eq:PoA_LQ}), when $r\to \infty$, we have $ \frac{(b_2 + \bar{b}_2)^2}{r + \bar{r}(1 - \bar{s})^2}\int_0^T (\Delta \bar{\eta}_t \cdot \bar{x}_t^{MFG})^2 dt \rightarrow 0$	and thus, $\displaystyle \lim_{r \to \infty} PoA = 1$. Idem, by replacing $\lambda^{\infty,r}$ with $\lambda^{\infty,\bar{r}}: = \frac{b_2}{b_2 + \bar{b}_2} (1 - \bar{s})$, we obtain $\displaystyle \lim_{\bar{r} \to \infty} PoA = 1$.
...
\begin{proof}
	...
	Moreover, since $B^{0,b_2}>0, B^{0,b_2}C^w >0, B^{0,b_2}D^w \geq 0$, we still have the unique solution for $w^{0,b_2}_t$ when $b_2 \to 0$. Since $B^{\eta} \to 0$, using the same argument shown in Proposition \ref{prop:r_bar_r}, we get:
	\begin{equation*}
	\begin{split}
	...
	For the shake of simplicity, let's denote $\displaystyle \kappa_u = \sqrt{\frac{q + \bar{q}(1-s)}{r + \bar{r}(1 - \bar{s})} } $ and $\zeta_1 = r + \bar{r}(1-\bar{s})^2$.
