...
\begin{equation*}
\begin{split}
\dot{\bar{\eta}}^{MFG}_t+(a^y_t+a^{\bar{y}}_t) (\bar{\eta}^{MFG}_t)^2+(a^x_t+a^{\bar{x}}_t-b^y_t-b^{\bar{y}}_t) \bar{\eta}^{MFG}_t -b^x_t-b^{\bar{x}}_t&=0,\\
\bar{\eta}^{MFG}_T-c^x-c^{\bar{x}}&=0,
\end{split}
\end{equation*}
\begin{equation*}
\begin{split}
\dot{\eta}^{MFG}_t+a^y_t(\eta^{MFG}_t)^2+(a^x_t-b^y_t)\eta^{MFG}_t-b^x_t&=0, \\
\eta^{MFG}_T&=c^x,
\end{split}
\end{equation*}
\begin{equation*}
\bar{x}^{MFG}_t=\mathbb{E}(\xi) e^{\int_0^t(a^x_u+a^{\bar{x}}_u+(a^y_u+a^{\bar{y}}_u)\bar{\eta}^{MFG}_u)du},
\end{equation*}
\begin{equation*}
v_t=Var(\xi)e^{\int_0^t 2(a^x_s+a^y_s\eta^{MFG}_s)ds}+\sigma^2 \int_0^t e^{2 \int_s^t (a^x_u+a^y_u\eta^{MFG}_u) du}ds.
\end{equation*}
...
\begin{equation*}
\begin{split}
\dot{\bar{\eta}}^{MKV}_t+(a^y_t+a^{\bar{y}}_t) (\bar{\eta}^{MKV}_t)^2+(a^x_t+a^{\bar{x}}_t-b^y_t-b^{\bar{y}}_t) \bar{\eta}^{MKV}_t -b^x_t-b^{\bar{x}}_t&=0,\\
\bar{\eta}^{MKV}_T-c^x-c^{\bar{x}}&=0,
\end{split}
\end{equation*}
\begin{equation*}
\begin{split}
\dot{\eta}^{MKV}_t+a^y_t(\eta^{MKV}_t)^2+(a^x_t-b^y_t)\eta^{MKV}_t-b^x_t&=0, \\
\eta^{MKV}_T&=c^x,
\end{split}
\end{equation*}
\begin{equation*}
\bar{x}^{MKV}_t=\mathbb{E}(\xi) e^{\int_0^t(a^x_u+a^{\bar{x}}_u+(a^y_u+a^{\bar{y}}_u)\bar{\eta}^{MKV}_u)du},
\end{equation*}
\begin{equation*}
v_t=Var(\xi)e^{\int_0^t 2(a^x_s+a^y_s\eta^{MKV}_s)ds}+\sigma^2 \int_0^t e^{2 \int_s^t (a^x_u+a^y_u\eta^{MKV}_u) du}ds.
\end{equation*}
