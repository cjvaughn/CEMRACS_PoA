...
%	$$
	...
	Next, we derive a uniform lower bound for $(b_2 + \bar{b}_2) u_t^{b_2}$ over $[0,T]$ and large $b_2$. Let $\epsilon_1 > 0$ so that $\epsilon_1 < \min \left\{ c_{\delta_u^{b_2 \to \infty}}, D^{u,b_2 \to \infty} \right\}$. Then there exists $b_2^{*,lower}>0$ such that for all $b_2 \geq b_2^{*,lower}$,
	$$ 
	...
	Now we are ready to show that $\lim_{b_2 \to \infty} \frac{\Delta SC}{SC^{MKV}} = 0$. Our approach is to split the interval $[0,T]$ into two parts, $[0,t^*] \cup [t^*,T]$.	
	Since $v_t \geq 0$ for all $t\in [0,T]$, we have $SC^{MKV} \geq w_0^{b_2} (\mathbb{E}(\xi))^2$.	For the sake of simplicity, we denote $h_u(b_2,t) := (b_2 + \bar{b}_2) u_{t}^{b_2}$ and $h_w(b_2,t) := (b_2 + \bar{b}_2)w_t^{b_2}$. We have:
	\begin{IEEEeqnarray*}{rCl}
	...
	$$	
	The time point $t^*$ will be chosen such that for any given $\epsilon>0$, we will have $I_1 \leq \epsilon$ and $I_2 \leq \epsilon$ for large $b_2$.
	  

	% the term I_1
	First, consider $I_1$. Recall that for $t \in[0,T/2]$, we have $\lim_{b_2 \to \infty}h_u(b_2,t) = c_u$ and $\lim_{b_2 \to \infty}h_w(b_2,t) = c_w$, and for all $b_2 \geq b_w^{*,upper}$, the functions $[0,T/2] \ni s \mapsto u_s^{b_2}$ and $[0,T/2] \ni s \mapsto w_s^{b_2}$ are increasing. Consequently, for any $\epsilon_2>0$ with $\epsilon_2 < \min \{ c_u, c_w \} $, there exists $b_2^{*,I_1}  > b_2^{*,upper}$ such that for all $b_2 \geq b_2^{*,I_1}$ and all $s\in [0, T/2]$ we have:
	\begin{equation*}
	\left\{
	...
	\right.
	\end{equation*}
	Thus, if $t^* < \frac{T}{2}$, then for any $t \in [0,t^*] \subset [0,T/2]$ and $b_2 \geq b_2^{*,I_1}$, we have:
	$$
	...
	which implies:
	\begin{IEEEeqnarray}{rCl}
		I_1 &\leq& \frac{e^{2T(b_1 + \bar{b}_1)}\cdot  2[ (c_u+ \epsilon_2)^2 + (c_w + \epsilon_2)^2 ]}{r+ \bar{r}(1-\bar{s})^2} \cdot (b_2 + \bar{b}_2) \int_0^{t^*} \exp \left(  - \frac{2 (b_2 + \bar{b}_2)}{r + \bar{r}(1-\bar{s})^2} \cdot (c_u - \epsilon_2) t \right) dt \nonumber \\
		&=& \kappa_1 \left( 1 - e^{ - \kappa_2 (b_2 + \bar{b}_2) t^* } \right), 
	\label{eq:I1_ineq_b2_infinity}
	\end{IEEEeqnarray}
	where the coefficients $ \displaystyle \kappa_1 := \kappa_1(\epsilon_2) = \frac{e^{2T(b_1 + \bar{b}_1)}\cdot  [ (c_u+ \epsilon_2)^2 + (c_w + \epsilon_2)^2 ]}{c_u - \epsilon_2}>0$ and $\displaystyle \kappa_2 := \kappa_2(\epsilon_2) = \frac{2 (c_u - \epsilon_2)}{r + \bar{r}(1-\bar{s})^2}>0$ are independent of $b_2$.	
	In particular, the above inequality is valid when $b_2 = b_2^{*,I_1}$, so if we choose the time point $t^*$ as follows:
	\begin{equation}
	...
	\begin{equation}
		I_1 \leq \kappa_1 \left(1 - e^{- \kappa_2 (b_2^{*,I_2} + \bar{b}_2) t^*}  \right) \leq \epsilon.\\
