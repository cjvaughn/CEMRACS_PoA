...
\begin{proof}
	...
	Moreover, since $\bar{x}_t^{MFG} = \mathbb{E}(\xi) \exp(\int_0^t (b_1 + \bar{b}_1 - B\cdot u_s)ds)$ and $v_t = Var(\xi)\exp(2\int_0^t (b_1 - B^\eta \eta_s) ds) + \sigma^2 \int_0^t \exp(2 \int_s^t (b_1 - B^\eta \eta_u) du) ds$, by the bounded convergence theorem we have for each $t \in [0,T]$: 
	...
	and thus,
	\begin{equation*}
	\lim_{r \to \infty} \frac{(b_2 + \bar{b}_2)^2}{r + \bar{r}(1 - \bar{s})^2}\int_0^T (\Delta \bar{\eta}_t \cdot \bar{x}_t^{MFG})^2 dt = 0.
	\end{equation*}
	...
	\end{equation*}
	Therefore, from Proposition \ref{prop:diff_SC} and equation (\ref{eq:PoA_LQ}), we have $\displaystyle \lim_{r \to \infty} PoA = 1$. Idem, by replacing $\lambda^{\infty,r}$ with $\lambda^{\infty,\bar{r}}: = \frac{b_2}{b_2 + \bar{b}_2} (1 - \bar{s})$, we obtain $\displaystyle \lim_{\bar{r} \to \infty} PoA = 1$.
