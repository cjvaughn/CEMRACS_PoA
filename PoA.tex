...
\end{proposition}
\begin{proof}
	When assumption $(\ref{eq:assumptions})$ is satisfied, we have the existence and uniqueness of solutions to the scalar Riccati equations associated with $u^{r}_t$, $w^{r}_t$ and $\eta^{r}_t$ for every given $r >0$. The superscript $r$ here indicates the dependence on this coefficient because we intend to study the behavior of these solutions in the asymptotic regime.	
	...
	In order to show that the $PoA$ tends to $1$, we can indeed show that $\Delta SC^{r}$ tends towards $0$ while $SC^{r, MKV} >0$ in the asymptotic regime. From the equation (\ref{eq:PoA_LQ}), a sufficient condition is to demonstrate that $\int_0^T (u^r_t - w^r_t)^2 (\bar{x}_t^{r,MFG} )^2 dt$ is finite and the variance function $v^r_t$ is strictly positive in the asymptotic regime.\\

	% modification Main point 5 (nicolas)
	If $A^u \neq 0$, let $u^{r \to \infty}:[0,T] \to \mathbb{R}$ be the solution to the first-order differential equation:
	\begin{equation}
		\dot{\left({u}^{r \to \infty}_t\right)} - 2A^u u_t^{r \to \infty} + C^{r \to \infty}_u = 0, \qquad u_T^{r \to \infty} = D^{r \to \infty}_u,
	\end{equation}
	or if $A^u = 0$, let $u^{r \to \infty}:[0,T] \to \mathbb{R}$ solve:
	\begin{equation}
		\dot{\left(u^{r\to \infty}_t\right)} + C^{r \to \infty}_u = 0, \qquad u_T^{r \to \infty} = D^{r \to \infty}_u.
	\end{equation}	
	Since assumption (\ref{eq:assumptions}) holds for arbitrarily large $r$, from the explicit formula of the solution to the scalar Riccati equation (see Appendix \ref{appendix_solving_fbsdes} equation (\ref{eq:riccati_ut_sol_app})), it is easy to show that the solution $u^r$ to equation (\ref{eq:riccati_ut}) converges pointwise for every time $t\in [0,T]$ to the solution $u^{r \to \infty}$ as long as $r \to \infty$. This limiting solution $u^{r \to \infty}$ can be given by the following expressions: for every $t\in [0,T]$
	\begin{equation*}
	\begin{split}
	    \text{when } A^u \neq 0,\qquad & u_t^{r \to \infty} = \left(D^{r \to \infty}_u - \frac{C^{r \to \infty}_u}{2A^u} \right) e^{-2A^u (T-t)} +  \frac{C^{r \to \infty}_u}{2A^u},\\
	    \text{when } A^u = 0, \qquad & u_t^{r \to \infty} = D^{r \to \infty}_u + C^{r \to \infty}_u (T-t).
	\end{split}
	\end{equation*}
	
	From the formula (\ref{eq:riccati_ut}), we can bound the solution $u^r$ uniformly over $[0,T]$ for large enough $r$. More precisely, under assumption (\ref{eq:assumptions}), we have $B^r>0, C^{u,r}>0, D^{u,r}>0$, and $\delta^{+,r}_u > 0 > \delta^{-,r}_u$. In the asymptotic regime, we have $\lambda^{r\to \infty} >0$, $C^{r \to \infty}_u >0$ and $D^{r \to \infty}_u \geq 0$. Thus, for $r$ large enough, there exists a constant $\zeta > 0 $ such that $\max\{ B^{r}, C^{u,r}, D^{u,r} \} \leq \zeta$. We also observe the following two inequalities when $A^u \neq 0$:
	\begin{IEEEeqnarray*}{rl}
		...
		< & C^{u,r} + D^{u,r} (\delta^{+,r}_u - \delta^{-,r}_u)\\
		= & C^{u,r} + D^{u,r} 2 \sqrt{ (A^u)^2 + B^{r} C^{u,r}} \\
		\leq&  \zeta + 2 \zeta \sqrt{(A^u)^2 + \zeta^2}  
	\end{IEEEeqnarray*}
	and 
	\begin{IEEEeqnarray*}{rl}
		...
		\geq & \max \left\{ - \delta^{-,r}_u , \delta_u^{+,r} e^{2 \sqrt{(A^u)^2 + B^{r} C^{u,r}} (T-t) } \right\} \\
		= & \max \left\{ A^u + \sqrt{ (A^u)^2 + B^{r} C^{u,r} } , (-A^u + \sqrt{ (A^u)^2 + B^{r} C^{u,r} } )  e^{- 2 \sqrt{(A^u)^2 + B^{r} C^{u,r}} (T-t) } \right\} \\
		\geq & \max \left\{ A^u + \vert A^u \vert, (-A^u + \vert A_u \vert) e^{-2 \sqrt{(A^u)^2 + \zeta^2} (T-t) } \right\}\\
		> & 0
	\end{IEEEeqnarray*}
	
	Thus, for $A^u \neq 0$, we can deduce that for $r$ large enough, for every $t \in [0,T]$
	\begin{IEEEeqnarray}{rCl}
		\vert u_t^{r} \vert  &=& \frac{\left\vert -(C^{u,r} + D^{u,r} \delta^{+,r}_u) + (C^{u,r} + D^{u,r} \delta^{-,r}_u) e^{- (\delta^{+,r}_u - \delta^{-,r}_u) (T-t)} \right\vert}{\left\vert ( \delta^{-,r}_u - D^{u,r} B^{r} ) - (\delta^{+,r}_u - D^{u,r} B^{r} ) e^{-(\delta^{+,r}_u - \delta^{-,r}_u)(T-t)} \right\vert } \nonumber \\
		& < & \frac{ \zeta + 2 \zeta \sqrt{(A^u)^2 + \zeta^2}  }{ \max \left\{ A^u + \vert A^u \vert, (-A^u + \vert A_u \vert) e^{-2 \sqrt{(A^u)^2 + \zeta^2} (T-t) } \right\} }  \nonumber \\
		& \leq & \min \{ \kappa, \kappa e^{\kappa (T-t)} \}  \nonumber \\
		&\leq & \kappa e^{\kappa T}
	\end{IEEEeqnarray}
	...
	\begin{equation}
		\kappa = \left\{
			\begin{array}{ll}
				\frac{\zeta + 2 \zeta \sqrt{(A^u)^2 + \zeta^2}}{A^u + \vert A^u \vert}, & if \quad A^u > 0 \\
				\max\left\{ \frac{\zeta + 2 \zeta \sqrt{(A^u)^2+ \zeta^2} }{ -A^u + \vert A_u \vert }, 2 \sqrt{ (A^u)^2 + \zeta^2 } \right\} & if \quad A^u < 0
			\end{array}
		\right.
	\end{equation}
	Thus, 
	Since $\bar{x}_t^{r,MFG} = \mathbb{E}(\xi) \exp(\int_0^t (b_1 + \bar{b}_1 - B^r \cdot u^r_s)ds)$,  by bounded convergence theorem we have for every $t \in [0,T]$,
	\begin{equation*}
		\bar{x}_t^{r,MFG} \xrightarrow[r\to \infty]{} \mathbb{E}(\xi) e^{(b_1+ \bar{b}_1)t} =: \bar{x}_t^{r\to \infty,MFG},
	\end{equation*}
	and moreover for $r$ large enough, $\bar{x}_t^{r,MFG}$ is uniformly bounded over $[0,T]$, i.e. 
	$$ \vert \bar{x}^{r,MFG}_t \vert \leq \vert \mathbb{E}[\xi] \vert e^{ (\vert{b_1} + \vert{\bar{b}_1} + \zeta \kappa e^{\kappa T} ) T }, \quad t \in [0,T]. $$  
	
	Similarly, for any fixed time $t\in [0,T]$, the solutions $w^r_t$ and $\eta^r_t$ of equations (\ref{eq:riccati_wt}) and (\ref{eq:riccati_etat}) converge for each time $t\in [0,T]$ towards the solutions $w^{r\to \infty}_t$ and $\eta_t^{r\to \infty}$ of some first-order differential equations. Namely, for every $t\in [0,T]$,
	$$w^r_t \xrightarrow[r \to \infty]{} w_t^{r\to \infty},\qquad \eta^r_t \xrightarrow[r \to \infty]{} \eta_t^{r\to \infty}$$
	and the functions $w^{r}$ and $\eta^{r}$ are uniformly bounded over $[0,T]$ for $r$ large enough.
	...
	and thus
	\begin{equation*}
	\lim_{r \to \infty} \Delta SC^r = \lim_{r \to \infty} \frac{1}{2} \frac{(b_2 + \bar{b}_2)^2}{r + \bar{r}(1 - \bar{s})^2}\int_0^T (u^r_t - w^r_t)^2 ( \bar{x}_t^{r,MFG})^2 dt = 0.
	\end{equation*}
	
	%---------------------------
	Also, since $v^r_t = Var(\xi)\exp(2\int_0^t (b_1 - B^{\eta,r} \eta^r_s) ds) + \sigma^2 \int_0^t \exp(2 \int_s^t (b_1 - B^{\eta,r} \eta^r_u) du) ds$, by the bounded convergence theorem we have for each $t \in [0,T]$: 
	$$	v^r_t \xrightarrow[r\to \infty]{} Var(\xi)e^{2b_1 t} + \sigma^2 \int_0^t e^{2 b_1 (t-s) } ds =: v^{r \to \infty}_t,
	$$
	The variance function $v_t^{r}$ is also uniformly bounded over $[0,T]$ for $r$ large enough.	It can be shown that $w^{r \to \infty }_0\geq0$ and $v^{r \to \infty}_t >0$ for $t>0$. Hence,
	\begin{equation*}
		\lim_{r \to \infty} SC^{MKV} = \frac{1}{2} \left( \int_0^T (q+\bar{q}) v^{r \to \infty}_t dt + (q_T + \bar{q}_T) v^{r\to \infty}_T + w^{r \to \infty}_0 (\mathbb{E}(\xi))^2 \right)  >0.
	\end{equation*}
	Therefore, from Proposition \ref{prop:diff_SC} and equation (\ref{eq:PoA_LQ}), we have $\displaystyle \lim_{r \to \infty} PoA = 1$. Idem, by replacing $\lambda^{r \to \infty}$ with $\lambda^{\bar{r} \to \infty}: = \frac{b_2}{b_2 + \bar{b}_2} (1 - \bar{s})$, we obtain $\displaystyle \lim_{\bar{r} \to \infty} PoA = 1$.
...
\begin{proof}
	...
	Using the fact that $w^{0,b_2}_t\not \equiv 0$  and together with the equation (\ref{eq:SC_MKV_2}), we have $0< \lim_{b_2\to 0} SC^{MKV} <\infty $ .
	...
	Now we consider the case when $b_2 \to \infty$. It can be seen later in the proof that both $SC^{MFG}$ and $SC^{MKV}$ tend towards $0$. In order to study the limit of the PoA,  the idea becomes to estimate the order of magnitude in terms of $b_2$ for the quantity $\Delta SC$ and $SC^{MKV}$.
	...
	and 
	\begin{IEEEeqnarray*}{rCl}
		\frac{\delta_u^{\pm}}{b_2 + \bar{b}_2} &=& \frac{A^u}{b_2 + \bar{b}_2} \pm \sqrt{\frac{C^u}{r + \bar{r}(1-\bar{s})^2} + \frac{(A^u)^2}{(b_2+\bar{b}_2)^2} }\\
		&=& \frac{A^u}{b_2} \left( 1 - \frac{\bar{b}_2}{b_2} + \mathcal{O}(b_2^{-2}) \right) \pm \sqrt{\kappa_u^2 \left( 1 - \frac{\bar{b}_2}{b_2} + \mathcal{O}(b_2^{-2}) \right) + \mathcal{O}(b_2^{-2}) } \\
		&=& \pm \kappa_u + \left(A^u \mp \frac{\bar{b}_2}{2}\kappa_u \right) \frac{1}{b_2} + \mathcal{O}(b_2^{-2})
	\end{IEEEeqnarray*}
	...
	The next step is to give an estimation of $\Delta SC$. To do so, we need to expand the product $B u_t$ up till $b_2^{-2}$. We first notice that for $t \in [0,T)$, 
	\begin{IEEEeqnarray*}{rCl}
		(b_2 + \bar{b}_2) u_t &=& \frac{ \displaystyle - \left(\frac{C^u}{b_2 + \bar{b}_2} + D^u . \frac{\delta^+_u}{b_2 + \bar{b}_2}\right) + \left(\frac{C^u}{b_2 + \bar{b}_2} + D^u . \frac{\delta^-_u}{b_2 + \bar{b}_2}\right)e^{- (\delta^+_u - \delta^-_u)(T-t)} }
		{\displaystyle \left(\frac{\delta^-_u}{(b_2 + \bar{b}_2)^2} - \frac{D^u}{r + \bar{r}(1-\bar{s})^2} \right) - \left(\frac{\delta^+_u}{(b_2 + \bar{b}_2)^2} - \frac{D^u}{r + \bar{r}(1-\bar{s})^2} \right)e^{- (\delta^+_u - \delta^-_u)(T-t)}  } \nonumber \\
		&=& \frac{ - (\mathcal{O}(b_2^{-1}) + \zeta_1 \frac{q_T + \bar{q}_T(1-s)}{r + \bar{r}(1-\bar{s})} \kappa_u + \mathcal{O}(b_2^{-1}) ) + \mathcal{O}(b_2^{-2})}{(\mathcal{O}(b_2^{-1})- \frac{q_T + \bar{q}_T(1-s)}{r + \bar{r}(1-\bar{s})} ) - \mathcal{O}(b_2^{-2}) } \nonumber \\
		&=& \zeta_1 \kappa_u + \mathcal{O}(b_2^{-1})
	\end{IEEEeqnarray*}
	Then, we can continue do the expansion and obtain
	\begin{IEEEeqnarray*}{rCl}
		&&(b_2 + \bar{b}_2) ( (b_2 + \bar{b}_2) u_t - \zeta_1 \kappa_u ) \nonumber\\
		&=& \frac{-(C^u + D^u \delta^+_u) + \mathcal{O}(b_2^{-2}) }{\displaystyle \left(\frac{\delta^-_u}{(b_2 + \bar{b}_2)^2} - \frac{D^u}{r + \bar{r}(1-\bar{s})^2} \right) + \mathcal{O}(b_2^{-2}) } - (b_2 + \bar{b}_2) \zeta_1 \kappa_u \nonumber \\
		&=& \frac{-\left( C^u + D^u \delta_u^+ + (b_2 + \bar{b}_2) \zeta_1 \kappa_u \left( \frac{\delta^-_u}{(b_2 + \bar{b}_2)^2} - \frac{D^u}{\zeta_1} \right) \right) + \mathcal{O}(b_2^{-2})}{  \left(\frac{\delta^-_u}{(b_2 + \bar{b}_2)^2} - \frac{D^u}{\zeta_1} \right)-  \mathcal{O}(b_2^{-2})} \nonumber \\
		&=& \frac{  -\left( C^u +\zeta_1 \kappa_u \frac{\delta_u^-}{b_2 + \bar{b}_2} +  D^u (b_2 + \bar{b}_2)(\frac{\delta^+_u}{b_2 + \bar{b}_2} - \kappa_u) \right) + \mathcal{O}(b_2^{-2}) }{ - \frac{D^u}{\zeta_1} + \mathcal{O}(b_2^{-1}) } \nonumber \\
		&=& \frac{- \left( \zeta_1 \kappa_u^2 (1 -\frac{\bar{b}_2}{b_2} )  + \zeta_1 \kappa_u (- \kappa_u + (A^u + \frac{\bar{b}_2}{2} \kappa_u) \frac{1}{b_2}) ) + D^u (b_2 + \bar{b}_2) \left[ (A^u - \frac{\bar{b}_2}{2} \kappa_u )\frac{1}{b_2} + \mathcal{O}(b_2^{-2}) \right] \right) + \mathcal{O}(b_2^{-2}) }{ - \frac{D^u}{\zeta_1} + \mathcal{O}(b_2^{-1})} \nonumber \\
		&=& \zeta_1 (A^u - \frac{\bar{b}_2}{2} \kappa_u ) + \mathcal{O}(b_2^{-1})
	\end{IEEEeqnarray*}
	...
	Thus
	\begin{IEEEeqnarray*}{rCl}
		\Delta SC &=& \frac{1}{2} \frac{(b_2+\bar{b}_2)^2}{\zeta_1} \int_0^T (u_t - w_t)^2 (\bar{x}_t^{MFG})^2 dt \\
		&=& \frac{\mathbb{E}(\xi)^2}{2} \int_0^T \left[ (\sqrt{\zeta_1 \kappa_u^2} - \sqrt{q+\bar{q}(1-s)^2}) + \mathcal{O}(b_2^{-1})\right]^2 . e^{2 (b_1+\bar{b}_1 - \kappa_u (b_2 + \bar{b}_2) - (A^u - \bar{b}_2 \kappa_u / 2) ) t} e^{-2 \int_0^t h_s(b_2) ds} dt\\
		...
		&& + \frac{\mathbb{E}(\xi)^2  (\sqrt{\zeta_1 \kappa_u^2} - \sqrt{q+\bar{q}(1-s)^2}) ^2}{2} \int_0^T \left[ (1 + \mathcal{O}(b_2^{-1}) ) e^{-2 \int_0^t h_s(b_2)} -1\right]. e^{2 (b_1+\bar{b}_1 - \kappa_u (b_2 + \bar{b}_2) - (A^u - \bar{b}_2 \kappa_u / 2) ) t}  dt
	\end{IEEEeqnarray*}
...
\begin{proof}
	...
	and 
	$$B \to B^{0,\bar{b}_2} := \frac{b_2^2}{r + \bar{r}(1-\bar{s})^2} > 0,\ C^u \to C^{0,\bar{b}_2}_u >0 ,\ D^u \to D^{0,\bar{b}_2}_u \geq 0.$$
	...
	$u^{0,\bar{b}_2}_t, w^{0,\bar{b}_2}_t$. By the bounded convergence theorem, the mean function $\bar{x}_t^{MFG}$ has the limit: (ToDo reference equation)
