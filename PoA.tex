...
\begin{proof}
	...
	Since $u_t^{\bar{b}_2}$, $w_t^{\bar{b}_2}$ and $\bar{x}_t^{MFG,\bar{b}_2}$ are continuous over $[0,T]$, then the functions $t \mapsto \int_0^t u^{\bar{b}_2}_s ds$ and $t \mapsto \int_0^t (u^{\bar{b}_2}_s- w^{\bar{b}_2}_s)^2 (\bar{x}_s^{MFG,\bar{b}_2} )^2 ds$ are continuous over $[0,T]$ and differentiable over $(0,T)$. Then with equation (\ref{eq:x_bar_MFG_explicit_new_notation}) we can apply the mean value theorem and deduce that there exists two fixed times $0< t_2 < t_1 < T$ such that:
	\begin{IEEEeqnarray}{rCl}
		...
		&=& \frac{T  B^{\bar{b}_2} }{2} \cdot (u_{t_1}^{\bar{b}_2}- w_{t_1}^{\bar{b}_2} )^2 \cdot (\mathbb{E}\xi)^2 e^{ 2 (b_1 + \bar{b}_1) t_1 - 2\int_0^{t_1} B^{\bar{b}_2} u_s^{\bar{b}_2} ds } \nonumber \\
		&=&  \kappa_1 (u_{t_1}^{\bar{b}_2}- w_{t_1}^{\bar{b}_2} )^2\cdot \left[ (b_2 + \bar{b}_2) e^{- B^{\bar{b}_2} u_{t_2}^{\bar{b}_2} \cdot t_1}  \right]^2 \nonumber \\
	...
	\end{IEEEeqnarray}
	where  $\kappa = \frac{t_1}{r + \bar{r}(1-\bar{s})^2} >0 $ and $\kappa_1 =\frac{T (\mathbb{E}\xi)^2 e^{2 (b_1 + \bar{b}_1) t_1}}{2 (r + \bar{r}(1-\bar{s})^2)} >0$ are two positive constants independent of $\bar{b}_2$, and $h(\bar{b}_2) = (b_2 + \bar{b}_2)^{\frac{3}{2}} u_{t_2}^{\bar{b}_2}$. (ToDo: $t_1(\bar{b}_2)$ and $t_2(\bar{b}_2)$.)
