...
\begin{IEEEeqnarray}{rCl}
	...
	\label{eq:v_t_MKV}
\end{IEEEeqnarray}
Note that while equations (\ref{eq:eta_bar_MKV})-(\ref{eq:v_t_MKV}) are quite similar to equations (\ref{eq:eta_bar_MFG})-(\ref{eq:v_t}) from the previous section, there are some subtle differences.
Now we have $SC^{MKV}=SC(\phi)$ where $\phi$ is the feedback function specified by this solution, namely, from equation (\ref{eq:alpha_hat_MKV}), we have:
\begin{equation*}
