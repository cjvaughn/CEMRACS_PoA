...
\begin{proof}
	...
	\begin{IEEEeqnarray}{rCl}
		...
		&= & \kappa_1 (u_{t_1}^{\bar{b}_2} - w_{t_1}^{\bar{b}_2} )^2 \cdot \left((b_2 + \bar{b}_2) e^{- \kappa (b_2 + \bar{b}_2)^{\frac{1}{2}} f(\bar{b}_2)} \right)^2
		\label{eq:delta_SC_b_2_bar_infty}
	\end{IEEEeqnarray}
	where  $\kappa = \frac{t_1}{r + \bar{r}(1-\bar{s})^2} >0 $ and $\kappa_1 =\frac{T \mathbb{E}[\xi]^2 e^{2 (b_1 + \bar{b}_1) t_1}}{2 (r + \bar{r}(1-\bar{s})^2)} >0$ are two positive constants independent of $\bar{b}_2$, and $f(\bar{b}_2) = (b_2 + \bar{b}_2)^{\frac{3}{2}} u_{t_2}^{\bar{b}_2}$. 
	

	
	Our next step is to show that $(b_2 + \bar{b}_2) e^{- \kappa (b_2 + \bar{b}_2)^{\frac{1}{2}} f(\bar{b}_2) } $ tends towards $0$ when $\bar{b}_2 \to \infty$. This can be done fairly easy. Since $f(\bar{b}_2) \to c_3 >0$, there exists an $\epsilon > 0 $ such that $c_3 - 2 \epsilon >0$ and a constant $\bar{b}_2^* := \bar{b}_2^*(\epsilon)>0$ so that for all $\bar{b}_2 \geq \bar{b}_2^*$ we have $\vert f(\bar{b}_2) - c_3 \vert < \epsilon$. Then $f(\bar{b}_2) - (c_3 - 2\epsilon) > \epsilon$ and 
	\begin{equation*}
	\begin{array}{rll}
		(b_2 + \bar{b}_2) e^{- \kappa (b_2 + \bar{b}_2)^{\frac{1}{2}} f(\bar{b}_2) }  &=& e^{- \kappa (b_2 + \bar{b}_2)^{\frac{1}{2}} (f(\bar{b}_2) - c_3 + 2\epsilon) }\cdot e^{- \kappa  (c_3 - 2 \epsilon) (b_2 + \bar{b}_2)^{\frac{1}{2}} +  \ln (b_2 + \bar{b}_2) } \\
		& \leq & e^{- \kappa (b_2+\bar{b}_2^*)^{\frac{1}{2}} \cdot \epsilon} \cdot e^{- \kappa (c_3 - 2 \epsilon) (b_2 + \bar{b}_2)^{\frac{1}{2}} + \ln (b_2 + \bar{b}_2)} 
	\end{array}
	\end{equation*}
	Then, for any $\epsilon_1>0$, we can choose $\bar{b}_2$ large enough, say $\bar{b}_2 \geq \bar{b}_2^{**} > \bar{b}_2^*$, so that
	\begin{equation}
	(b_2 + \bar{b}_2) e^{- \kappa (b_2 + \bar{b}_2)^{\frac{1}{2}} f(\bar{b}_2) }   \leq \epsilon_1.
	...
	\end{equation}
 	...
 	then for any $\epsilon >0$ there exists a $\bar{b}_2^* > 1$ such that for any $\bar{b}_2 \geq \bar{b}_2^*$ we have $ (b_2 + \bar{b}_2)^{\frac{3}{2}} \vert u_{t_1}^{\bar{b}_2} \vert \leq c_3 + \epsilon$. Thus, for any $\bar{b}_2 \geq \bar{b}_2^*$ we deduce
