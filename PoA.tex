...
\begin{proof}
	...
	\end{equation}
	Denote $h_u(b_1,t) = B \frac{u_t^{b_1}}{b_1}$ and $h_w(b_1,t) = B \frac{w_t^{b_1}}{b_1}$. From equation (\ref{eq:riccati_ut_sol_deriv}), and together with equation (\ref{eq:ut_wt_lim_b1_infty}), there exists $b_1^{*,upper} >0$ such that for all $b_1 \geq b_1^{*,upper}$, we have:
	$$\max \left\{ \left|\frac{u_0^{b_1}}{b_1}-\frac{2}{B} \right|,\left|\frac{w_0^{b_1}}{b_1}-\frac{2}{B} \right| \right\} \leq \frac{1}{B},$$
	and the functions $t \mapsto u_t^{b_1}$ and $t \mapsto w_t^{b_1}$ are decreasing. Thus,
	...
	Fix $\epsilon>0$. There exists $b^{*,I_1}_1\geq b_1^{*,upper}$ such that for all $b_1 \geq b_1^{*,I_1} $, and for all $t \in [0, 3/4 T]$,
	...
	We adapt the methodology in Proposition \ref{prop:b2_to_infty} and split the interval $[0,T]$ into two parts: $[0, 3/4 T]$ and $[3/4 T, T]$. For all $b_1 \geq b_1^{*,I_1}(\epsilon)$, similar to equation (\ref{eq:I_1_plus_I_2}), we have:
	\begin{equation*}
