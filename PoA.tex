...
\begin{proof}
	...
	\end{IEEEeqnarray*}
	where $\kappa_1 =\frac{T \mathbb{E}[\xi]^2 e^{2 (b_1 + \bar{b}_1) t_1}}{2 (r + \bar{r}(1-\bar{s})^2)} >0$ is a positive constant independent of $\bar{b}_2$. 
	
	Let's denote $(b_2 + \bar{b}_2)^{\frac{1}{2}} = p$ and $f(p) = p^3 u_{t_2}^{\bar{b}_2}$, then we know that $f(p) \xrightarrow[p\to \infty]{} \delta^{\bar{b}_2 \to \infty} (r + \bar{r}(1- \bar{s})^2) := c_3 > 0$. Then we have
	\begin{equation}
		 \Delta SC = \kappa_1 (u_{t_1}^{\bar{b}_2}- w_{t_1}^{\bar{b}_2} )^2 \cdot \left(p^2 e^{- \kappa p f(p)} \right)^2
	\label{eq:delta_SC_b_2_bar_infty}
	\end{equation}
	where $\kappa = \frac{t_1}{r + \bar{r}(1-\bar{s})^2} >0 $.
